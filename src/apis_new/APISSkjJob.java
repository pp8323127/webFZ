 ...
 * 20140402新增 發送越南Arv,HAN&SGN
 * 20140613 美國加入address
 * 20140624 調整 TDT段:(每個航段皆有TDT-LOC-DTM-LOC-DTM) ,BGM
 * 20140704 調整 TDT段:(每個航段皆有TDT-LOC-DTM-LOC-DTM) 修正 V2
 * 20140808 調整日本回覆舊版TDT段
 * 20141015新增KIX dep/arv both send. 
 * 2014/10/31 USA ?->空格
 * 2014/11/21 RUS over fly
 */
...
{   
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
	        ...
	        c.getAPISFlt("2013/04/15","0591","KHH");
//	        c.getAPISFlt("2011/12/12","0065","BKK");
//	        c.getAPISFlt("2011/04/19","5391","SEA");
	        ArrayList apisfltAL = new ArrayList();
...
//			            }
  ...
  //check need to send over fly sector
    public String CheckOverFly(String sect,String city){
        Statement stmt = null;
        ResultSet rs = null;
...
//             stmt = conn.createStatement();  
             ...
             sql = " select sect from fztoverFly where city = '"+city+"' and flag = 'Y' "; 
// System.out.println("apis over = "+sql);            
             rs = stmt.executeQuery(sql);
             ArrayList objAL = new ArrayList();  
             while (rs.next())
             {
                 objAL.add(rs.getString("sect"));             
             }
             if(objAL.size()>0){
                 for(int i=0 ; i<objAL.size() ; i++){
                     if(sect.equals(objAL.get(i).toString())){
                         returnstr = "Y";
                         i = objAL.size();   
                     }
                 }
    ...
    //人工發送 depend on port local time 
    //fdate formate: yyyy/mm/dd
    public String getAPISFlt(String fdate, String fltno, String dpt)
    {   
        Statement stmt = null;
        ResultSet rs = null;
        Connection conn = null;
...
//	    	stmt = conn.createStatement();	
	    	...
	    	sql = " SELECT flt_num fltno,  fdate,  dpt, arv, carrier, " +
	    		  ...
	    		  " WHERE str_port_local between To_Date('"+fdate+" 00:00','yyyy/mm/dd hh24:mi') " +
	    		  " AND To_Date('"+fdate+" 23:59','yyyy/mm/dd hh24:mi') " +
	    		  " AND  (dpt=scity OR arv = scity) " +
	    		  " AND ctry IN ('USA','CHINA','CANADA','JAPAN','KOREA','INDIA','UK','NEW ZEALAND','TAIWAN','NETHERLANDS') " +
	    		  " AND flt_num ='"+fltno+"' " +
	    		  " AND dpt = '"+dpt+"' GROUP BY  flt_num, fdate, dpt,arv, carrier, str_port_local, end_port_local, stdtpe,  " +
...
//System.out.println(sql);	  
	    	...
	    	while (rs.next())
	    	{
	    	    ...
	    	    obj.setFly_status(rs.getString("fly_status"));
	    	    //**********************************************
	    	    //Get DA13 data	    	    
...
//	    	stmt = conn.createStatement();	
	    	...
	    	sql = " select da13_acno, da13_leg_seq_nr, da13_fltno, da13_op_suffix, da13_fm_sector, da13_to_sector, da13_actp, " +
		  		  ...
		  		  " to_char(da13_stdl,'yyyy/mm/dd hh24:mi') da13_stdl, " +
		  		  " to_char(da13_stal,'yyyy/mm/dd hh24:mi') da13_stal " +		  		  
		  		  " from v_ittda13_ci " +
		  		  " where da13_cond NOT IN ('CF','DL','OF','RR') " +
		  		  " and (LPAD(da13_fltno||trim(da13_op_suffix),5,'0')= LPad('"+fltno+"',5,'0') " +
		  		  " or LPAD(da13_fltno,5,'0')= LPad('"+fltno+"',5,'0'))" +
...
//System.out.println("DA13 sql =  "+sql);	    	
	    	...
	    	while (rs.next())
	    	{
	    	    ...
	    	    obj.setDa13_stdl(rs.getString("da13_stdl"));
	    	    obj.setDa13_stal(rs.getString("da13_stal"));
     ...
     //stdtpe = fdatetime 2010/05/13 01:01
     //public ArrayList getAPISFltDetail_DA13(String fltno, String stdtpe, String exphr, String dpt)
     //str_port_local = fdatetime 2010/05/13 01:01
     public ArrayList getAPISFltDetail_DA13(String fltno, String str_port_local, String exphr, String dpt)
     {   
         Statement stmt = null;
         ResultSet rs = null;
         Connection conn = null;
...
//		  		  " to_char(da13_stdu+8/24,'yyyy/mm/dd hh24:mi') da13_stdu " +
//		  		  " from v_ittda13_ci " +
//		  		  " where da13_cond NOT IN ('CF','DL','OF') " +
//		  		  " and (LPAD(da13_fltno||trim(da13_op_suffix),5,'0')= LPad('"+fltno+"',5,'0') " +
//		  		  " or LPAD(da13_fltno,5,'0')= LPad('"+fltno+"',5,'0')) " + 		  
//		  		  " and da13_stdu+8/24 between to_date('"+stdtpe+"','yyyy/mm/dd hh24:mi')-("+exphr+"/24) " +
//		  		  " and to_date('"+stdtpe+"','yyyy/mm/dd hh24:mi')+("+exphr+"/24) and da13_fm_sector = '"+dpt+"'" +
//		  		  " order by da13_leg_seq_nr asc";
	    	...
	    	sql = " select da13_acno, da13_leg_seq_nr, da13_fltno, da13_op_suffix, da13_fm_sector, da13_to_sector, da13_actp, " +
		  		  ...
		  		  " from v_ittda13_ci " +
		  		  " where da13_cond NOT IN ('CF','DL','OF','RR') " +
		  		  " and DA13_SCDATE_U between to_date('"+str_port_local+"','yyyy/mm/dd hh24:mi') -30 AND  to_date('"+str_port_local+"','yyyy/mm/dd hh24:mi')+5 " +
		  		  " and (LPAD(da13_fltno||trim(da13_op_suffix),5,'0')= LPad('"+fltno+"',5,'0') " +
		  		  " or LPAD(da13_fltno,5,'0')= LPad('"+fltno+"',5,'0')) " + 		  
		  		  " and da13_etdl between to_date('"+str_port_local+"','yyyy/mm/dd hh24:mi')-("+exphr+"/24) " +
		  		  " and to_date('"+str_port_local+"','yyyy/mm/dd hh24:mi')+("+exphr+"/24) and da13_fm_sector = '"+dpt+"'" +
		  		  " order by da13_leg_seq_nr asc";
	    	
//System.out.println("DA132 sql =  "+sql);	 
	    	rs = stmt.executeQuery(sql);
	    	while (rs.next())
	    	{
	    	    DA13Obj obj = new DA13Obj();
...
//         System.out.println("apisdetailAL.size() = "+apisdetailAL.size());
         ...
         {  
	         ...
	         da13objAL = obj.getDa13AL();
	     	 //發報address
			 //大陸:BJSCCXH(海關)、PEKKN1E(邊防)  CHINA
			 //印尼:CGKBCXH  INDONESIA
			 //美國：DCALPCR USA //20140915 DCAUCCR --> DCALPCR
			 //加拿大：YOWAAXH CANADA
			 //韓國：SELTSXA  KOREA
			 //印度：FABINXS  INDIA
	         //日本:TYONAXH  JAPAN (入境才需要/出境無需要),[20130805]只有HND皆要發送
	         
	         //Departure
//	         if("CHINA".equals(portobj1.getCtry()))
//	         {
//	             qd_str = " BJSCCXH PEKKN1E TPEWGCI";
//	             ischina = true;
//	         }
...
//	         }
	         ...
	         {
	             qd_str = " DCALPCR TPEWGCI";
	         }
	         else if("CANADA".equals(portobj1.getCtry()))
	         {
	             qd_str =// " YOWAAXH TPEWGCI LONCBXS";
	                     " TPEWGCI";//20130301 del--LONCAXS
	         }         
	       ...
	       //20140808改透過getJANDepAPISTxtHT發送
//	         else if("JAPAN".equals(portobj1.getCtry()))
//             {
//                 //20130805新增HND dep/arv both send. 
//                 if("HND".equals(obj.getDpt())){
...
//				         txtSB.append("NAD(MS(((ATTN? "+obj.getDpt()+"KK"+obj.getCarrier()+")"+"");
                         ...
                         {                             
                             ...
                             {
                                 boolean relat_main_ctry = true;
                                 //set D A R
                                 DA13Obj da13obj = (DA13Obj) obj.getDa13AL().get(d);
                                 //若dpt.ctry != arv.ctry && arv.ctry == main_ctry 
                                 //此為國家交界,APIS 先D 後A
                                 PortCityObj portobj3 = pc.getPortCityObj(da13obj.getDa13_fm_sector());//dpt sector
                                 PortCityObj portobj4 = pc.getPortCityObj(da13obj.getDa13_to_sector());//arv sector  
                                 
                                 if(!portobj3.getCtry().equals(portobj4.getCtry()) && (portobj3.getCtry().equals(main_ctry) | portobj4.getCtry().equals(main_ctry)))
                                 {//入出境 main country 之 international flt
                                     da13obj.setDpt_status_type("D");
                                     da13obj.setArv_status_type("A");
                                 }
                                 else if(!portobj3.getCtry().equals(portobj4.getCtry()))
                                 {//和 main country 無關之航段
                                     da13obj.setDpt_status_type("D");
                                     da13obj.setArv_status_type("A");    
                                     relat_main_ctry = false;
                                 }
                                 else if (portobj3.getCtry().equals(main_ctry) | portobj4.getCtry().equals(main_ctry))
                                 {//domestic flt
                                 ...
                                 if(relat_main_ctry == true)
                                 {
                                     if("R".equals(da13obj.getArv_status_type()))
                                     {//domestic flt
                                         txtSB.append("TDT(20("+obj.getCarrier()+tempfltno+")"+"");                                             
                                         txtSB.append("LOC(92("+da13obj.getDa13_fm_sector()+")"+"");                                                
                                         txtSB.append("DTM(189:"+da13obj.getDa13_atdl().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201)"+"");
                                         txtSB.append("LOC(92("+da13obj.getDa13_to_sector()+")"+"");                                              
                                         txtSB.append("DTM(232:"+da13obj.getDa13_atal().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201)"+"");
                                         line_cnt = line_cnt +5;
                                     }
                                     else if(portobj3.getCtry().equals(main_ctry))
                                     {//internation flt 出境
                                         txtSB.append("TDT(20("+obj.getCarrier()+tempfltno+")"+"");                                             
                                     ...
                                     else if(portobj4.getCtry().equals(main_ctry))
                                     {//internation flt 入境
                                         txtSB.append("TDT(20("+obj.getCarrier()+tempfltno+")"+"");                                             
                                         txtSB.append("LOC(125("+da13obj.getDa13_fm_sector()+")"+"");                                                
                                         txtSB.append("DTM(189:"+da13obj.getDa13_atdl().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201)"+"");
...
//					                             line_cnt++;
//					                         }	
//					                         //2011/04/18 JFKSEA CI5391 Case 加else
//					                         //*************************************
//					                         else
//					                         {
//					                             if("R".equals(da13obj.getArv_status_type()) && da13objAL.size()<=1)
//					                             {
//					                                 txtSB.append("LOC(92("+da13obj.getDa13_to_sector()+")"+"");
//					                                 txtSB.append("DTM(232:"+tempobj.getDa13_atal().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201)"+"");
//					                                 line_cnt = line_cnt +2;
...
//				             }
				            //不送address 20140613 mark 
//				             txtSB.append("NAD(FM((("+apisdetailobj.getLname().replaceAll("[?]","??").replaceAll(" ","? ").replaceAll("[(]","?(").replaceAll("[)]","?)").replaceAll("[.]","?.")+":"+apisdetailobj.getFname().replaceAll("[?]","??").replaceAll(" ","? ").replaceAll("[(]","?(").replaceAll("[)]","?)").replaceAll("[.]","?.")+")"+"");

                             /*2014/06/13 add address start */
                             if(apisdetailobj.getResiaddr3()==null | "".equals(apisdetailobj.getResiaddr3()))
                             {
                                 //txtSB.append("NAD(FM((("+apisdetailobj.getLname().replaceAll("[?]","??").replaceAll(" ","?").replaceAll("[(]","?(").replaceAll("[)]","?)").replaceAll("[.]","?.")+":"+apisdetailobj.getFname().replaceAll("[?]","??").replaceAll(" ","?").replaceAll("[(]","?(").replaceAll("[)]","?)").replaceAll("[.]","?.")+"("+apisdetailobj.getResiaddr1().replaceAll("[?]","??").replaceAll("[.]","?.").replaceAll(" ","?").replaceAll("[(]","?(").replaceAll("[)]","?)")+"("+apisdetailobj.getResiaddr2().replaceAll("[?]","??").replaceAll("[.]","?.").replaceAll(" ","?").replaceAll("[(]","?(").replaceAll("[)]","?)")+"(("+apisdetailobj.getResiaddr4().replaceAll("[?]","??").replaceAll(" ","?").replaceAll("[.]","?.").replaceAll("[(]","?(").replaceAll("[)]","?)")+"("+apisdetailobj.getResiaddr5().replaceAll("[?]","??").replaceAll("[.]","?.").replaceAll(" ","?").replaceAll("[(]","?(").replaceAll("[)]","?)")+")"+"");
                                 //2014/10/31 USA ?->空格
                                 txtSB.append("NAD(FM((("+apisdetailobj.getLname().replaceAll("[?]"," ").replaceAll("[(]","(").replaceAll("[)]",")").replaceAll("[.]",".")
                                         +":"+apisdetailobj.getFname().replaceAll("[?]"," ").replaceAll("[(]","(").replaceAll("[)]",")").replaceAll("[.]",".")
                                         +"("+apisdetailobj.getResiaddr1().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"("+apisdetailobj.getResiaddr2().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"(("+apisdetailobj.getResiaddr4().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"("+apisdetailobj.getResiaddr5().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")+")"+"");
                             ...
                             else
                             {
                               ...
                               //2014/10/31 USA ?->空格
                                 txtSB.append("NAD(FM((("+apisdetailobj.getLname().replaceAll("[?]"," ").replaceAll("[(]","(").replaceAll("[)]",")").replaceAll("[.]",".")
                                         ...
                                         +"("+apisdetailobj.getResiaddr1().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"("+apisdetailobj.getResiaddr2().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"("+apisdetailobj.getResiaddr3().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"("+apisdetailobj.getResiaddr4().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")
                                         +"("+apisdetailobj.getResiaddr5().replaceAll("[?]"," ").replaceAll("[.]",".").replaceAll("[(]","(").replaceAll("[)]",")")+")"+"");
                             }
                             /*2014/06/13 add address end */
...
//					         2014/10/31 USA ?->空格
					         ...
					         //20130118
					         if(!"CANADA".equals(portobj1.getCtry())){
...
//			   }
			   ...
			   //20140808改透過getJANArvAPISTxtHT發送
...
//                       /*20140624針對TDT修正 V1 */
//                          txtSB.append("TDT(20("+obj.getCarrier()+tempfltno+")"+"");/*20140624*/
//                         line_cnt =5;         
//                         
//                         if(obj.getDa13AL().size()>0)
//                         {
...
//                           2014/10/31 USA ?->空格
					         txtSB.append("LOC(180("+apisdetailobj.getBirthcountry()+"(:::"+apisdetailobj.getBirthcity().trim().replaceAll("[?]"," ").replaceAll("[(]","(").replaceAll("[)]",")").replaceAll("[.]",".")+")"+"");
					         //20130118
					         if(!"CANADA".equals(portobj2.getCtry())){
					             txtSB.append("EMP(1("+apisdetailobj.getTvlstatus()+":110:111)"+"");
					         }
					         txtSB.append("NAT(2("+apisdetailobj.getPasscountry()+")"+"");
					         txtSB.append("RFF(AVF:CCRR11)"+"");
...
//=======20140122=======
//       修改APIS(手動及自動)威海(WEH)航班:
//       四碼之航班編號, 若第一碼為0, 則去除, 例: 0539/0540改為539/540.
//       UNH與TDT段都有航班編號.
         if(tempfltno.length()>=4 && ("WEH".equals(obj.getDpt()) || "WEH".equals(obj.getArv())))
         {
             tempfltno=tempfltno.substring(1,4);
         }
//===================== 
         try
         {  
	         ...
	         if("CHINA".equals(portobj1.getCtry()))
	         {
	             if("PEK".equals(obj.getDpt())){//2013/04 北京加發PEKDPCA
	                 qd_str = " BJSCCXH PEKKN1E TPEWGCI PEKDPCA"; 
	             }else{
	                 qd_str = " BJSCCXH PEKKN1E TPEWGCI";
	             }
	         }
...
//===================== 
         try
         {
	         ...
	         //Arrival
			   ...
			   {
			       //qd_str = " BJSCCXH PEKKN1E TPEWGCI";
			       if("PEK".equals(obj.getArv())){//2013/04 北京加發PEKDPCA
	                     qd_str = " BJSCCXH PEKKN1E TPEWGCI PEKDPCA"; 
	               }else{
	                     qd_str = " BJSCCXH PEKKN1E TPEWGCI";
	               }
...
//   荷蘭 Netherland NL
     //*************************************************************************************
     public Hashtable getNLDptAPISTxtHT(APISObj obj, ArrayList apisdetailAL)
     {//Dpt~Arv (outbound)   
//         System.out.println("apisdetailAL.size() = "+apisdetailAL.size());
         errorstr = "";
         Hashtable txtHT = new Hashtable();      
	...
	//     	 boolean isemboard = false;
	         ...
	         //荷蘭 Netherlands NL         
	         //Departure
	         if("NETHERLANDS".equals(portobj1.getCtry()))
	         {
//	             qd_str = " LHRKMXA TPEWGCI";//test
	             qd_str = " HDQKMXH TPEWGCI";//live
	         }
	...
	//         }
		...
		//System.out.println("crew_cnt"+apisdetailAL.size()+" ** "+ telex_page_start +"  ** "+telex_page_end);             
			         ...
			         {         
			             ...
			             txtSB.append("UNB+UNOA:4+TPETT"+obj.getCarrier()+":"+obj.getCarrier()+"+NLDAPIKM:NL+"+time1+"+"+time2+"++APIS'"+"\r\n");
			             txtSB.append("UNG+PAXLST+CHINA AIRLINES:"+obj.getCarrier()+"+NLDAPIKM:NL+"+time1+"+"+time2+"+UN+D:02B'"+"\r\n");
				         txtSB.append("UNH+PAX001+PAXLST:D:02B:UN:IATA+"+obj.getCarrier()+tempfltno+"/"+obj.getEnd_port_local().replaceAll("/","").replaceAll(":","").replaceAll(" ","/").substring(2)+"+"+page_str+"'"+"\r\n");
				         if("BEF".equals(obj.getFly_status()))
				         {
				             txtSB.append("BGM+250'"+"\r\n");
...
//				                     }			                   
				                 ...
				                 {//"D".equals(da13obj.getDpt_status_type())
				                     ...
				                     for(int c=0; c<obj.getDa13AL().size(); c++)
				                     {
				                         ...
				                         if(tempobj.getDa13_fm_sector().equals(da13obj.getDa13_fm_sector()))
				                         {
				                             txtSB.append("DTM+189:"+tempobj.getDa13_stdl().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201'"+"\r\n");
				                             line_cnt++;
				                         }
				                         if(tempobj.getDa13_to_sector().equals(da13obj.getDa13_fm_sector()))
				                         {
				                             txtSB.append("DTM+232:"+tempobj.getDa13_stal().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201'"+"\r\n");
				                             line_cnt++;
				                         }				                         
				                     }
...
//System.out.println(txtSB.toString());
     ...
     //荷蘭 Netherland NL
     //*********************************************************************************************
     public Hashtable getNLArvAPISTxtHT(APISObj obj, ArrayList apisdetailAL)
     {//Dpt~Arv (inbound) 
         errorstr = "";
         Hashtable txtHT = new Hashtable();  
		 String tempfltno = obj.getFltno();
         ...
         try
         {
			 ...
			 //荷蘭：NETHERLANDS 
	         //Arrival
	         if("NETHERLANDS".equals(portobj2.getCtry()))
	         {
//	             qd_str = " LHRKMXA TPEWGCI";//test
	             qd_str = " HDQKMXH TPEWGCI";//live
	         }
	...
	//       Cgo& not domestic ==> B
		...
		//System.out.println("crew_cnt"+apisdetailAL.size()+" ** "+ telex_page_start +"  ** "+telex_page_end);             
			         ...
			         {         
			             ...
			             txtSB.append("UNB+UNOA:4+TPETTCI:CI+NLDAPIKM:NL+"+time1+"+"+time2+"++APIS'"+"\r\n");
			             txtSB.append("UNG+PAXLST+CHINA AIRLINES:CI+NLDAPIKM:NL+"+time1+"+"+time2+"+UN+D:02B'"+"\r\n");
				         txtSB.append("UNH+PAX001+PAXLST:D:02B:UN:IATA+"+obj.getCarrier()+tempfltno+"/"+obj.getEnd_port_local().replaceAll("/","").replaceAll(":","").replaceAll(" ","/").substring(2)+"+"+page_str+"'"+"\r\n");
...
//System.out.println(txtSB.toString());
     ...
     //越南VIETNAM  HAN/HANOI ; SGN/HO CHI MING CITY
     //*********************************************************************************************
     public Hashtable getVNIArvAPISTxtHT(APISObj obj, ArrayList apisdetailAL){
       //Dpt~Arv (inbound) 
         errorstr = "";
         Hashtable txtHT = new Hashtable();  
         String tempfltno = obj.getFltno();
         ...
         try
         {
             ...
             //Arrival
               if("VIETNAM".equals(portobj2.getCtry()))
               {                 
                   //20140402新增 
                     if("HAN".equals(obj.getArv())){
//                         qd_str = " HANATXH TPEWGCI";//test
                         qd_str = " HANAPXH TPEWGCI";//live
                     }else if ("SGN".equals(obj.getArv())){
//                         qd_str = " SGNATXH TPEWGCI";//test
                         qd_str = " SGNAPXH TPEWGCI";//live
                     }else if ("DAD".equals(obj.getArv())){
                         qd_str = "DADSDXH TPEWGCI";//live
                     }else{
                         qd_str = "";
                     }
               }
    ...
    //       Cgo& not domestic ==> B
        ...
        //System.out.println("crew_cnt"+apisdetailAL.size()+" ** "+ telex_page_start +"  ** "+telex_page_end);             
                     ...
                     {         
                         ...
                         txtSB.append("UNG+PAXLST+CHINA AIRLINES:ZZ+VNIMM:ZZ+"+time1+"+"+time2+"+UN+D:02B'"+"\r\n");
                         //txtSB.append("UNH+PAX001+PAXLST:D:02B:UN:IATA+"+obj.getCarrier()+tempfltno+"/"+obj.getEnd_port_local().replaceAll("/","").replaceAll(":","").replaceAll(" ","/").substring(2)+"+"+page_str+"'"+"\r\n");
                                 ...
                                 {                               
                                     if(getDA == false)
                                     {
                                         ...
                                         for(int c=0; c<obj.getDa13AL().size(); c++)
                                         {
                                             ...
                                             {
                                                 //correct when MIA case
//                                               txtSB.append("DTM+189:"+tempobj.getDa13_stal().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201'"+"\r\n");
                                                 txtSB.append("DTM+232:"+tempobj.getDa13_stal().replaceAll("/","").replaceAll(":","").replaceAll(" ","").substring(2)+":201'"+"\r\n");
                                                 line_cnt++;
                                             }                                       
                                         }
...
//System.out.println(txtSB.toString());
     ...
     //日本Japan 由於美國格式修改,日本暫只吃舊格式  20140808修改
     //*********************************************************************************************
     public Hashtable getJANDptAPISTxtHT(APISObj obj, ArrayList apisdetailAL)
     {//Dpt~Arv (outbound)   
//         System.out.println("apisdetailAL.size() = "+apisdetailAL.size());
         errorstr = "";
         Hashtable txtHT = new Hashtable();      
...
//                 qd_str = " FABINXS TPEWGCI";
//             }else 
             if("JAPAN".equals(portobj1.getCtry()))
             {
                 //20130805新增HND dep/arv both send. 
                 //20141015新增KIX dep/arv both send. 
                 if("HND".equals(obj.getDpt()) || "KIX".equals(obj.getDpt())){
                     qd_str = " TYONAXH TPEWGCI";
                 }else{
                     qd_str = "";
                 }
    ...
    //         }
        ...
        //System.out.println("crew_cnt"+apisdetailAL.size()+" ** "+ telex_page_start +"  ** "+telex_page_end);             
                     ...
                     {         
                         ...
                         txtSB.append("NAD(MS(((ATTN? "+obj.getDpt()+"KK"+obj.getCarrier()+")"+"");
                         txtSB.append("COM(011-8863-398-3989:TE(011-8863-399-8194:FX)"+"");
                         /*20140704針對TDT修正 V1, 0808rollback */
                       txtSB.append("TDT(20("+obj.getCarrier()+tempfltno+")"+"");
...
//                       System.out.println(txtSB.toString());
     ...
     //日本Japan 由於美國格式修改,日本暫只吃舊格式  20140808修改
     //*********************************************************************************************
     public Hashtable getJANArvAPISTxtHT(APISObj obj, ArrayList apisdetailAL)
     {//Dpt~Arv (inbound) 
         errorstr = "";
         Hashtable txtHT = new Hashtable();  
         String tempfltno = obj.getFltno();
...
//                   qd_str = " FABINXS TPEWGCI";
//               }
               if("JAPAN".equals(portobj2.getCtry()))
               {
                   qd_str = " TYONAXH TPEWGCI";               
               }
             //****************************************************************
...
//                             txtSB.append("NAD(FM((("+apisdetailobj.getLname().replaceAll("[?]","??").replaceAll(" ","? ").replaceAll("[(]","?(").replaceAll("[)]","?)").replaceAll("[.]","?.")+":"+apisdetailobj.getFname().replaceAll("[?]","??").replaceAll(" ","? ").replaceAll("[(]","?(").replaceAll("[)]","?)").replaceAll("[.]","?.")+")"+"");
    ...
    //                   System.out.println(txtSB.toString());
     ...
     //俄羅斯RUSSIAN,飛越領空
     //*********************************************************************************************
     public Hashtable getRUSoverAPISTxtHT(APISObj obj, ArrayList apisdetailAL)
     {//Dpt~Arv (outbound)   
//         System.out.println("apisdetailAL.size() = "+apisdetailAL.size());
         errorstr = "";
         Hashtable txtHT = new Hashtable();      
    ...
    //         }
        ...
        //System.out.println("crew_cnt"+apisdetailAL.size()+" ** "+ telex_page_start +"  ** "+telex_page_end);             
                     ...
                     {         
                         ...
                         txtSB.append("UNB+UNOA:4+TPETT"+obj.getCarrier()+":ZZ+RUSAPIS:ZZ+"+time1+"+"+time2+"++APIS'"+"\r\n");
                         txtSB.append("UNG+PAXLST+CHINA AIRLINES:ZZ+RUSAPIS:ZZ+"+time1+"+"+time2+"+UN+D:02B'"+"\r\n");
                         txtSB.append("UNH+PAX001+PAXLST:D:02B:UN:IATA+"+obj.getCarrier()+time3+"+"+page_str+"'"+"\r\n");
                         if("BEF".equals(obj.getFly_status()))
                         {
                             txtSB.append("BGM+250'"+"\r\n");
...
//                       txtSB.append("NAD+MS+++ATTN "+obj.getDpt()+"KK"+obj.getCarrier()+"'"+"\r\n");
                         txtSB.append("NAD+MS+++PILOT DAILY COUNTER'"+"\r\n");
                         txtSB.append("COM+011-8863-398-3989:TE+011-8863-399-8194:FX'"+"\r\n");
                         txtSB.append("TDT+34+"+obj.getCarrier()+tempfltno+"'"+"\r\n");
                         line_cnt =5;  
...
//         System.out.println("apisdetailAL.size() = "+apisdetailAL.size());
	...
	//     	 boolean isemboard = false;
	         ...
	         {
//			       qd_str = " TPEATXH TPEWGCI";//test
	             qd_str = " TPEAPXH TPEWGCI";//live
	         }
