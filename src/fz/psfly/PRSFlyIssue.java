...
//			System.out.println(sql);
			...
			while(rs.next())
			{
			    topic_noAL.add(rs.getString("topic_no"));
			}
    ...
    public void getPsflyTopic_no_old(String fltd, String fltno, String dpt, String arv, String empno, String tempfleet, String tempacno) 
    {   
        Connection conn = null;
    	Statement stmt = null;
    	ResultSet rs = null;	
...
//				  " ORDER BY topic_no, itemno";		
            ...
            sql = " SELECT tb.topic_no topic_no, tb.itemno itemno, fi.itemdesc itemdesc, " +
            	  " fi.kin kin, fi.qtype qtype, ti.check_duty, ti.psfm_itemno " +
            	  " FROM egtpstb tb,egtpsfi fi, (SELECT topic_no, check_duty, psfm_itemno FROM egtpsti GROUP BY topic_no, check_duty,psfm_itemno) ti " +
            	  " WHERE tb.topic_no in ("+topic_no+") AND tb.itemno = fi.itemno and tb.topic_no = ti.topic_no " +
            	  " ORDER BY tb.topic_no , tb.itemno ";
...
//			System.out.println(sql);
			...
			while(rs.next())
			{
			    ...
			    obj.setCheck_duty(rs.getString("check_duty"));
    ...
    {   
        ...
        {           
			...
			sql = " SELECT tb.topic_no topic_no, tb.itemno itemno, fi.itemdesc itemdesc, fi.kin kin, " +
				  " fi.qtype qtype, ti.check_duty, ti.psfm_itemno, sf.* FROM egtpstb tb, egtpsfi fi , " +
				  " (SELECT topic_no, check_duty, psfm_itemno FROM egtpsti GROUP BY topic_no, check_duty, psfm_itemno) ti," +
				  ...
				  " and tb.topic_no = ti.topic_no AND tb.itemno = sf.itemno (+) ORDER BY tb.topic_no, tb.itemno ";			
//			System.out.println(sql);
			bankobjAL = new ArrayList();
			rs = stmt.executeQuery(sql);
			while(rs.next())
...
//			System.out.println(sql);
			...
			while(rs.next())
			{
			    ...
			    obj.setTopic_no(rs.getString("topic_no"));
			    obj.setItemno(rs.getString("itemno"));
			    prid.getBankItemData(rs.getString("psfm_itemno"));	   
			    prid.getFactorData(rs.getString("psfm_itemno"));
			    prid.getFactorSubData(rs.getString("psfm_itemno"));
			    obj.setItemdesc(prid.getBankItemDesc(rs.getString("itemno")));
//			    System.out.println(prid.getBankItemDesc(rs.getString("itemno")));
			    obj.setKin(rs.getString("psfm_itemno"));
			    obj.setQtype("S");
