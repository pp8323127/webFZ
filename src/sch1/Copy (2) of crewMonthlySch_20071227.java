...
public class crewMonthlySch {
	...
	public String schFile(String fmm, String base, String rank, String sk) {
		...
		try {
			fw = new FileWriter("/apsource/csap/projfz/webap/" + fmm + base
					+ rank + sk + ".csv", false);
			...
			if (!"".equals(rank) && "X".equals(sk))
				q_rank = " and cr.rank_cd='" + rank + "' ";
			if ("retire".equals(sk)) {
				q_sk = " and sk.skill_cd in ('*','**') ";
			} else if (!"X".equals(sk)) {
				q_sk = " and sk.skill_cd='" + sk + "' ";
			}
...
// sb
// .append("select cr.rank_cd,r.staff_num empno, c.preferred_name cname, ");
// sb.append("to_char(d.act_str_dt_tm_gmt,'dd') sdd, ");
// sb.append("to_char(d.act_end_dt_tm_gmt,'dd') edd, ");
// sb.append("to_char(d.act_str_dt_tm_gmt,'mm') smm, ");
// sb.append("to_char(d.act_end_dt_tm_gmt,'mm') emm, ");
// sb.append("to_char(d.act_str_dt_tm_gmt,'HH24MI') shh, ");
// sb.append("to_char(d.act_end_dt_tm_gmt,'HH24MI') ehh, ");
// sb.append("to_char(d.str_dt_tm_loc,'HH24') sloc, ");
// sb.append("lpad(nvl(d.flt_num,''),4,'0') fltno, ");
// sb
// .append("decode(d.duty_cd,'LO','---',nvl(d.duty_cd,'')) dutycd,
// nvl(d.cop_duty_cd,'') copcd, ");
// sb.append("d.port_a dpt, d.port_b arv, c.seniority_code code ");
// sb
// .append("from roster_v r, duty_prd_seg_v d, crew_v c, crew_base_v b,
// rank_tp_v rt, crew_rank_v cr, crew_spec_skills_v sk ");
// sb.append("where r.series_num=d.series_num ");
// sb.append("and r.staff_num=c.staff_num ");
// sb
// .append("and c.staff_num=b.staff_num and b.base=? and (b.exp_dt >= sysdate or
// b.exp_dt is null) ");
// sb
// .append("and r.acting_rank=rt.display_rank_cd and rt.fd_ind='N' ");
// sb
// .append("and c.staff_num=cr.staff_num and (cr.exp_dt >= sysdate or cr.exp_dt
// is null) ");
//
// sb
// .append("and c.staff_num=sk.staff_num(+) and (sk.exp_dt >= sysdate or
// sk.exp_dt is null) ");
// sb.append(q_sk);
// // "and r.staff_num='638395' " + //********
// sb
// .append("and (d.act_str_dt_tm_gmt between to_date(?,'yyyymmddHH24MI') and
// last_day(to_date(?,'yyyymmHH24MI')) ");
// sb
// .append("or d.act_end_dt_tm_gmt between to_date(?,'yyyymmddHH24MI') and
// last_day(to_date(?,'yyyymmHH24MI'))) ");
// sb.append("and r.delete_ind='N' and d.duty_cd<>'RST' ");
// sb.append(q_rank);
// sb.append("union all ");
//
// sb
// .append("select cr.rank_cd,r.staff_num empno, c.preferred_name cname, ");
// sb.append("to_char(r.act_str_dt,'dd') sdd, ");
// sb.append("to_char(r.act_end_dt,'dd') edd, ");
// sb.append("to_char(r.act_str_dt,'mm') smm, ");
// sb.append("to_char(r.act_end_dt,'mm') emm, ");
// sb.append("to_char(r.act_str_dt,'HH24MI') shh, ");
// sb.append("to_char(r.act_end_dt,'HH24MI') ehh, ");
// sb.append("to_char(r.act_str_dt,'HH24') sloc, ");
// sb.append("'' fltno, ");
// sb.append("r.duty_cd dutycd, '' copcd, ");
// sb
// .append("r.location_cd dpt, r.location_cd arv, c.seniority_code code ");
// sb
// .append("from roster_v r, crew_v c, crew_base_v b, rank_tp_v rt, crew_rank_v
// cr, crew_spec_skills_v sk ");
// sb.append("where r.series_num=0 ");
// sb.append("and r.staff_num=c.staff_num ");
// sb
// .append("and c.staff_num=b.staff_num and b.base=? and (b.exp_dt >= sysdate or
// b.exp_dt is null) ");
...
// // "and r.staff_num='638395' " + //********
// sb
// .append("and (r.act_str_dt between to_date(?,'yyyymmddHH24MI') and
// last_day(to_date(?,'yyyymmHH24MI')) ");
// sb
// .append("or r.act_end_dt between to_date(?,'yyyymmddHH24MI') and
// last_day(to_date(?,'yyyymmHH24MI'))) ");
// sb.append("and r.delete_ind='N' ");
// sb.append(q_rank);
// sb.append("order by code, empno, sdd, edd");
			...
			while (rs.next()) {
				...
				Big5Cname = new String(ci.tool.UnicodeStringParser
						.removeExtraEscape(cname).getBytes(), "Big5");
				sdd = rs.getInt("sdd");
				edd = rs.getInt("edd");
				smm = rs.getInt("smm");
				emm = rs.getInt("emm");
				...
				if (!empno.equals(kempno)) {
					...
					// ","+cr+","+rankCd+"\n";
					wstring = wstring.substring(0, wstring.length() - 1) + ","
							+ cr + "," + staffRankObj.getRankCd(kempno) + "\n";
				...
				if ("FLY".equals(dutycd) || "TVL".equals(dutycd)) {
					...
					if (smm == inmm) {
						if (sch[sdd - 1].length() >= 8)
							sch[sdd - 1] = sch[sdd - 1].substring(0,
									sch[sdd - 1].indexOf(" "))
									+ " ";
					}
					if (smm == inmm) {
						// loc time 凌晨02:00以前加'*'
						if (sloc < 2 && sch[sdd - 1].indexOf("*") < 0)
							sch[sdd - 1] = sch[sdd - 1] + "*";
