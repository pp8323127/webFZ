...
//        deleteAl we = new deleteAl();
//        String[] offsdate ={"2008/10/06"};
//        String[] offedate ={"2008/10/08"};
//        String[] offno ={"373233"};
        
//        System.out.println(we.deleteoff(offsdate, offedate, offno, "632168"));
...
//          stmt = con.createStatement(); 
			...
			for(int i = 0; i<offsdate.length; i++)
			{
				...
				if (dd <= 10)
				{
					//Add by Betty on 2008/09/10
					//***********************************************
					//判斷請假班表是否published
					boolean ifpublished = false;

					aa = " SELECT offsdate, newdate , pubdate, CASE when newdate>= pubdate " +
						 " THEN 'Y' ELSE 'N' END ifpublished FROM egtoffs, fztspub " +
						 " WHERE offno = '"+offno[i]+"'  AND  yyyy= To_Char(Add_Months(offsdate,-1),'yyyy') " +
						 " AND mm =To_Char(Add_Months(offsdate,-1),'mm') ";
//					System.out.println(aa);
		            myResultSet = stmt.executeQuery(aa);
	                if (myResultSet.next())
					{
						String str = myResultSet.getString("ifpublished").trim();
						if("Y".equals(str))
						{
							ifpublished = true;
						}
				...
				else
				{
					...
					int jobno = 0;
		                        ...
		                        aa = "select station, jobno, nvl(specialcode,'N') specialcode from egtcbas where trim(empn) = trim('"+empn+"')";
		                        myResultSet = stmt.executeQuery(aa);
	                                if (myResultSet.next())
	                                {
			                                        ...
			                                        for (int j = 0; j <= offdays; j++)
			                                        {
			                                                ...
			                                                else//非日語組組員, 韓語組組員
			                                                {
				                                                	...
				                                                	aa = "SELECT count(*) qlcount " +
					                                			     "FROM EGTCBAS, EGTOFFS  " +
					                                			     "WHERE ( EGTCBAS.empn = EGTOFFS.empn ) and " +   
					                                			     "( ( EGTCBAS.jobno = "+str1+" or EGTCBAS.jobno = "+str2+") AND " + 
					                                			     "( EGTCBAS.station = 'TPE' ) AND ( NVL(EGTCBAS.specialcode, 'N') not in ('J','K') ) AND " + 
					                                			     "( to_char(EGTOFFS.offsdate,'yyyy-mm-dd') <= '"+theday+"' ) AND " +  
					                                			     "( to_char(EGTOFFS.offedate,'yyyy-mm-dd') >= '"+theday+"' ) AND " +
					                                			     "( EGTOFFS.offtype = '0' )  and (EGTOFFS.remark <> '*' or EGTOFFS.remark is null))";
			                                                        }
			                                                        else  if (jobno==100 || jobno==120) //FS 100,120
			                                                        {
			                                                                str1 = "100";
			                                                                str2 = "120";
			                                                                //*****add by cs55 2005/07/19 取得當月Quota*****
			                                                        ...
			                                                        {			                                                    
			                                                                //*****add by cs55 2005/07/19 取得當月Quota*****
						                                                	myResultSet = stmt.executeQuery("select quota from egtquota where yy||'-'||mm=substr('"+theday+"',1,7) and qitem='TPE ZC'");
			                                                            	
						                                                	while(myResultSet.next()){
						                                                		zc = myResultSet.getInt("quota");
						                                                	}
				                                                	//**********************************************
				                                                	aa = "SELECT count(*) qlcount " +
					                                			     "FROM EGTCBAS, EGTOFFS  " +
					                                			     "WHERE ( EGTCBAS.empn = EGTOFFS.empn ) and " +   
					                                			     "( EGTCBAS.jobno = '95'  AND " + 					                                			     
					                                			     "( EGTCBAS.station = 'TPE' ) AND ( NVL(EGTCBAS.specialcode, 'N') not in ('J','K') ) AND " + 
					                                			     "( to_char(EGTOFFS.offsdate,'yyyy-mm-dd') <= '"+theday+"' ) AND " +  
					                                			     "( to_char(EGTOFFS.offedate,'yyyy-mm-dd') >= '"+theday+"' ) AND " +
					                                			     "( EGTOFFS.offtype = '0' )  and (EGTOFFS.remark <> '*' or EGTOFFS.remark is null))";
			                                                        ...
			                                                        if (myResultSet != null)
			                                                        {
			                                                        	...
			                                                        	{ 
			                                                        	        qlcount = myResultSet.getInt("qlcount");
			                                                        	        if ((jobno==110 || jobno==90) && qlcount >= fa)
			                                                        	        {
			                                                        	                return "TPE-FA "+theday+" had over "+fa+" , You can't cancel it.";
			                                                        	        }
			                                                        	        else if ((jobno==120 || jobno==100) && qlcount >= fs)
			                                                        	        {
			                                                        	                return "TPE-FS "+theday+" had over "+fs+" , You can't cancel it.";
			                                                        	        }
			                                                        	        else if (jobno==95 && qlcount >= zc)
			                                                        	        {
			                                                        	                return "Zone chief "+theday+" had over "+zc+" , You can't cancel it.";
			                                                        	        }
			                                                        	}
			                                                        }
			                                                }
