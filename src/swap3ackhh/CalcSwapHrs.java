...
public class CalcSwapHrs {
			...
			try {
				if (aSwapSkjIdx == null) {
					...
					for (int i = 0; i < aSwapCr.length; i++) {
						CrewSkjObj obj = (CrewSkjObj) aSkjAL.get(Integer
								.parseInt(aSwapSkjIdx[i]));
						aSwapCr[i] = obj.getCr();
						aSwapSkjAL.add(obj);
					}
				}
			...
			try {
				if (rSwapSkjIdx == null) {
					...
					for (int i = 0; i < rSwapCr.length; i++) {
						CrewSkjObj obj = (CrewSkjObj) rSkjAL.get(Integer
								.parseInt(rSwapSkjIdx[i]));
						rSwapCr[i] = obj.getCr();
						rSwapSkjAL.add(obj);
					}
				}
	...
	private void setSwaptotalCr(String[] aSwapCr, String[] rSwapCr) {
		...
		if (getTotalMin(rSwapCr) > getTotalMin(aSwapCr)) {
			aSwapDiffCr = TimeUtil.minToHHMM(Integer
					.toString(getTotalMin(rSwapCr) - getTotalMin(aSwapCr)));
			rSwapDiffCr = "-" + aSwapDiffCr;
		} else if (getTotalMin(rSwapCr) < getTotalMin(aSwapCr)) {
			rSwapDiffCr = TimeUtil.minToHHMM(Integer
					.toString(getTotalMin(aSwapCr) - getTotalMin(rSwapCr)));
	...
	private void setTotalSwapCr(String[] aSwapCr, String[] rSwapCr) {
		if (aSwapCr == null) {
			...
			aSwapTotalCr = TimeUtil.minToHHMM(Integer
					.toString(getTotalMin(aSwapCr)));
		}

		if (rSwapCr == null) {
			rSwapTotalCr = "0000";
		} else {
			rSwapTotalCr = TimeUtil.minToHHMM(Integer
					.toString(getTotalMin(rSwapCr)));
		}
