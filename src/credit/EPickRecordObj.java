package credit;

/**
 * @author CS71 Created on  2012/5/25
 */
public class EPickRecordObj
{
    private String  seq        ="";
    private String  empno      ="";
    private String  sern       ="";
    private String  cname      ="";
    private String  qual       ="";
    private String  grp        ="";
    private String  station    ="";
    private String  sno        ="";
    private String  skj_seq    ="";
    private String  preference ="";
    private String  ed_check   ="";
    private String  comments   ="";
    private String  new_user   ="";
    private String  new_date   ="";
    private String  check_user ="";
    private String  check_date ="";
    CreditObj creditobj        = null;
    
    public String getCheck_date()
    {
        return check_date;
    }
    public void setCheck_date(String check_date)
    {
        this.check_date = check_date;
    }
    public String getCheck_user()
    {
        return check_user;
    }
    public void setCheck_user(String check_user)
    {
        this.check_user = check_user;
    }
    public String getCname()
    {
        return cname;
    }
    public void setCname(String cname)
    {
        this.cname = cname;
    }
    public String getComments()
    {
        return comments;
    }
    public void setComments(String comments)
    {
        this.comments = comments;
    }
    public CreditObj getCreditobj()
    {
        return creditobj;
    }
    public void setCreditobj(CreditObj creditobj)
    {
        this.creditobj = creditobj;
    }
    public String getEd_check()
    {
        return ed_check;
    }
    public void setEd_check(String ed_check)
    {
        this.ed_check = ed_check;
    }
    public String getEmpno()
    {
        return empno;
    }
    public void setEmpno(String empno)
    {
        this.empno = empno;
    }
    public String getGrp()
    {
        return grp;
    }
    public void setGrp(String grp)
    {
        this.grp = grp;
    }
    public String getNew_date()
    {
        return new_date;
    }
    public void setNew_date(String new_date)
    {
        this.new_date = new_date;
    }
    public String getNew_user()
    {
        return new_user;
    }
    public void setNew_user(String new_user)
    {
        this.new_user = new_user;
    }
    public String getPreference()
    {
        return preference;
    }
    public void setPreference(String preference)
    {
        this.preference = preference;
    }
    public String getQual()
    {
        return qual;
    }
    public void setQual(String qual)
    {
        this.qual = qual;
    }
    public String getSeq()
    {
        return seq;
    }
    public void setSeq(String seq)
    {
        this.seq = seq;
    }
    public String getSern()
    {
        return sern;
    }
    public void setSern(String sern)
    {
        this.sern = sern;
    }
    public String getSkj_seq()
    {
        return skj_seq;
    }
    public void setSkj_seq(String skj_seq)
    {
        this.skj_seq = skj_seq;
    }
    public String getSno()
    {
        return sno;
    }
    public void setSno(String sno)
    {
        this.sno = sno;
    }
    public String getStation()
    {
        return station;
    }
    public void setStation(String station)
    {
        this.station = station;
    }
}
