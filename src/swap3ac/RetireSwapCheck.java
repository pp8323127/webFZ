...
//	}
	...
	private void setFlow() {
		...
		if (aPr > aCeiling) {
			...
			} else if (rPr >= 7000 && rPr <= rCeiling) {
				// flow = 12;

				if (aSwap <= aPr && (rSwap >= 7000 && rSwap <= rCeiling)) {
					setExchangeable(true);
				} else {
					setErrorMsg(2);
				}
		...
		} else if (aPr >= 7000 && aPr <= aCeiling) {
			if (rPr > rCeiling) {
				// flow = 21;
				if ((aSwap >= 7000 && aSwap <= aCeiling) && (rSwap <= rPr)) {
					setExchangeable(true);
				} else {
					setErrorMsg(4);
				}
				...
				// flow = 22;
				if ((aSwap >= 7000 && aSwap <= aCeiling)
						&& (rSwap >= 7000 && rSwap <= rCeiling)) {
					setExchangeable(true);
				} else {
					setErrorMsg(5);
				}
	...
	private void setErrorMsg(int errorCode) {
		...
		switch (errorCode) {
			case 1 :
				// errorMsg = "申請者與被換者換後飛時，需小於換前飛時.";
				errorMsg = "申請者與被換者,,換後飛時均需大於或小於換前飛時.";
				break;
			case 2 :
				// errorMsg = "申請者換後飛時需小於換前飛時,<br>被換者換後飛時，需介於100與70小時之間";

				if (aRetire && rRetire) {
					errorMsg = "申請者需換較低時數,<br>被換者換後飛時，需介於90與70小時之間";
				} else if (aRetire) {
					errorMsg = "申請者需換較低時數,<br>被換者換後飛時，需介於100與70小時之間";
				} else if (rRetire) {
					errorMsg = "申請者需換較低時數,<br>被換者換後飛時，需介於90與70小時之間";
				} else {
					errorMsg = "申請者需換較低時數,<br>被換者換後飛時，需介於100與70小時之間";
				...
				break;
			case 3 :
				// errorMsg = "申請者換後飛時需於換前飛時,<br>被換者換後飛時需大於換前飛時";
				errorMsg = "申請者需換較低時數,<br>被換者需換較高時數";
				break;
			case 4 :
				// errorMsg = "申請者換後飛時需介於100與70小時之間.<br>被換者換後飛時需小於換前飛時";

				if (aRetire && rRetire) {
					errorMsg = "申請者換後飛時需介於90與70小時之間.<br>被換者需換較低時數";
				} else if (aRetire) {
					errorMsg = "申請者換後飛時需介於90與70小時之間.<br>被換者需換較低時數";
				} else if (rRetire) {
					errorMsg = "申請者換後飛時需介於100與70小時之間.<br>被換者需換較低時數";
				} else {
					errorMsg = "申請者換後飛時需介於100與70小時之間.<br>被換者需換較低時數";
				}
			...
			case 5 :
				if (aRetire && rRetire) {
					errorMsg = "申請者與被換者,換後飛時均需介於90與70小時之間.";
				} else if (aRetire) {
					errorMsg = "申請者換後飛時均需介於90與70小時之間<br>"
							+ "被換者,換後飛時均需介於100與70小時之間.";

				} else if (rRetire) {
					errorMsg = "申請者換後飛時均需介於100與70小時之間<br>"
							+ "被換者,換後飛時均需介於90與70小時之間.";
				} else {
					errorMsg = "申請者與被換者,換後飛時均需介於100與70小時之間.";
				}

				break;
			case 6 :
				// errorMsg = "申請者換後飛時需介於100與70小時之間.<br>被換者換後飛時需大於換前飛時";
				if (aRetire && rRetire) {
					errorMsg = "申請者換後飛時需介於90與70小時之間.<br>被換者需換較高時數";
				} else if (aRetire) {
					errorMsg = "申請者換後飛時需介於90與70小時之間.<br>被換者需換較高時數";
				} else if (rRetire) {
					errorMsg = "申請者換後飛時需介於100與70小時之間.<br>被換者需換較高時數";
				} else {
					errorMsg = "申請者換後飛時需介於100與70小時之間.<br>被換者需換較高時數";
				}
				...
				break;
			case 7 :
				// errorMsg = "申請者換後飛時需大於換前飛時,<br>被換者換後飛時需小於換前飛時";
				errorMsg = "申請者需換較高時數,<br>被換者需換較低時數";
				break;
			case 8 :
				// errorMsg = "申請者換後飛時需大於換前飛時,<br>被換者換後飛時，需介於100與70小時之間";

				if (aRetire && rRetire) {
					errorMsg = "申請者需換較高時數,<br>被換者換後飛時，需介於90與70小時之間";
				} else if (aRetire) {
					errorMsg = "申請者需換較高時數,<br>被換者換後飛時，需介於100與70小時之間";
				} else if (rRetire) {
					errorMsg = "申請者需換較高時數,<br>被換者換後飛時，需介於90與70小時之間";
				} else {
					errorMsg = "申請者需換較高時數,<br>被換者換後飛時，需介於100與70小時之間";
