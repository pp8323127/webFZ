...
//System.out.println(" 申請者個人資料  select c.staff_num,sum(c.non_std_fly_hours) totalcr ,"
//		+ "crew.preferred_name cname,to_number(crew.seniority_code) sern,crank.rank_cd occu,"
//		+ "base.base,crew.section_number grps from crew_cum_hr_cc_v c,crew_v crew,"
//		+ "crew_base_v base, crew_rank_v crank where c.staff_num = crew.staff_num "
//		+ "AND c.staff_num = crank.staff_num AND c.staff_num= base.staff_num  "
...
//System.out.println("申請者取得CR= SELECT staff_num,sum(non_std_fly_hours) totalcr "
...
//System.out.println("被換者  select c.staff_num,sum(c.non_std_fly_hours) totalcr ,"
//		+ "crew.preferred_name cname,to_number(crew.seniority_code) sern,crank.rank_cd occu,"
//		+ "base.base,crew.section_number grps from crew_cum_hr_cc_v c,crew_v crew,"
//		+ "crew_base_v base, crew_rank_v crank where c.staff_num = crew.staff_num "
//		+ "AND c.staff_num = crank.staff_num AND c.staff_num= base.staff_num AND  "
//		+ "c.staff_num ='"+rEmpno+"' and to_char(c.cal_dt,'yyyymm')='"+year + month+"' "
//		+ "AND (base.exp_dt>SYSDATE OR base.exp_dt IS NULL) "
//		+ "AND base.prim_base='Y' "
//		+ "AND crank.eff_dt <=SYSDATE AND (crank.exp_dt >=SYSDATE OR crank.exp_dt IS null) "
//		+ "GROUP BY c.staff_num,crew.preferred_name,crew.seniority_code,"
...
//System.out.println("被換者CR= SELECT staff_num,sum(non_std_fly_hours) totalcr "
//		+ "FROM crew_cum_hr_cc_v c   WHERE staff_num = '"+rEmpno+"' AND  c.cal_dt  BETWEEN "
//		+ "To_Date('"+year + month + "01 0000"+"','yyyymmdd hh24mi')  AND Last_Day(To_Date('"+year + month + "01 0000"+"','yyyymmdd hh24mi')) GROUP BY  staff_num");				
...
//System.out.println("取得申請者班表 select r.staff_num,  To_Char(dps.series_num) tripno,"
//		+ "To_Char(str_dt_tm_loc,'yyyy/mm/dd') fdate,"
//		+ " dps.port_a dpt,"
//		+ " dps.port_b arv,"
//		// 加入day of week ,
...
//		+ "where dps.series_num=r.series_num "
//		+ "AND r.series_num <> 0 "// 必須有series_num
//		+ "AND r.staff_num='"+aEmpno+"' AND r.delete_ind='N' "
//		+ "AND dps.duty_seq_num||dps.item_seq_num=11 " // 只抓第1段
//		// + "AND dps.duty_cd NOT IN
//		// ('B1','EE','MT','CT','FT','B2','GS') "
//		+ "AND act_str_dt_tm_gmt "
...
//		+ "from roster_v 	WHERE str_dt "
//		+ dateRangeCondition
//		+ " AND delete_ind='N' AND series_num=0 	AND staff_num='"+aEmpno+"'"
//		+ " order by fdate");				
...
//System.out.println("申請者 取得各trip的cr "+(String) seriesAL.get(i)+"  select nvl(SUM(trunc(DECODE(duty_cd,'FLY', duration_mins,"
//		+ " duration_mins*0.5))),0) totalCr from duty_prd_seg_v"
//		+ " where (duty_cd='FLY' or cop_duty_cd='ACM') "
//		+ " and series_num='"+(String) seriesAL.get(i)+"' "
//		+ " and act_str_dt_tm_gmt <= Last_Day(To_Date('"+year+"/"+month+"/01 235959','yyyy/mm/dd hh24:mi:ss'))");							
...
//System.out.println("被換者班表	 select r.staff_num,  To_Char(dps.series_num) tripno,"
//		+ "To_Char(str_dt_tm_loc,'yyyy/mm/dd') fdate,"
//		+ " dps.port_a dpt,"
//		+ " dps.port_b arv,"
//		// 加入day of week ,
...
//		+ "where dps.series_num=r.series_num "
//		+ "AND r.series_num <> 0 "// 必須有series_num
//		+ "AND r.staff_num='"+rEmpno+"' AND r.delete_ind='N' "
//		+ "AND dps.duty_seq_num||dps.item_seq_num=11 " // 只抓第1段
//		// + "AND dps.duty_cd NOT IN
//		// ('B1','EE','MT','CT','FT','B2','GS') "
//		+ "AND act_str_dt_tm_gmt "
...
//		+ "from roster_v 	WHERE str_dt "
//		+ dateRangeCondition
//		+ " AND delete_ind='N' AND series_num=0 	AND staff_num='"+rEmpno+"'"
//		+ " order by fdate");				
...
//System.out.println("被換者取得各trip的cr "+(String) seriesAL.get(i)+"  select nvl(SUM(trunc(DECODE(duty_cd,'FLY', duration_mins,"
//		+ " duration_mins*0.5))),0) totalCr from duty_prd_seg_v"
//		+ " where (duty_cd='FLY' or cop_duty_cd='ACM') "
//		+ " and series_num='"+(String) seriesAL.get(i)+"' "
//		+ " and act_str_dt_tm_gmt <= Last_Day(To_Date('"+year+"/"+month+"/01 235959','yyyy/mm/dd hh24:mi:ss'))");							
