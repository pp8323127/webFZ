...
//					cn.getConnPW());
			...
			pstmt = conn.prepareStatement("select * from fztbform where formno = ?");
			...
			pstmt = conn.prepareStatement("select * from fztbaply where formno="
					+ formno + " and therole=?");
			// 申請者
			pstmt.setString(1, "A");
			rs = pstmt.executeQuery();
			...
			while (rs.next()) 
			{
				...
				skjObj.setActp(rs.getString("actp"));
				al.add(skjObj);
			}
