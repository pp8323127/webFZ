package eg;

/**
 * @author cs71 Created on  2007/3/3
 */
public class EgInfoObj
{
    private String empn          = "";
    private String sern 		 = "";
    private String section	 	 = "";
    private String cname		 = "";
    private String ename		 = "";
    private String deptno	     = "";
    private String jobno		 = "";
    private String hiredate 	 = "";
    private String indate 	     = "";
    private String status 	     = "";
    private String worktype 	 = "";
    private String ssdate 	     = "";
    private String station	 	 = "";
    private String id 		 	 = "";
    private String sex		 	 = "";
    private String birth		 = "";
    private String bplace	 	 = "";
    private String height	 	 = "";
    private String blood		 = "";
    private String nation	 	 = "";
    private String province 	 = "";
    private String city 		 = "";
    private String educate	 	 = "";
    private String groups 	 	 = "";
    private String cabin		 = "";
    private String green		 = "";
    private String passport 	 = "";
    private String passdate 	 = "";
    private String visadate 	 = "";
    private String cmcno		 = "";
    private String homeadrs 	 = "";
    private String mailadrs 	 = "";
    private String emailadrs	 = "";
    private String phone1 	 	 = "";
    private String phone2 	 	 = "";
    private String phone3 	 	 = "";
    private String phone4 	 	 = "";
    private String phone5 	 	 = "";
    private String emgname	 	 = "";
    private String emgphone1	 = "";
    private String emgphone2	 = "";
    private String resignno 	 = "";
    private String rsndate	 	 = "";
    private String newdept	 	 = "";
    private String comments 	 = "";
    private String newuser	 	 = "";
    private String newdate	 	 = "";
    private String chguser	 	 = "";
    private String chgdate	 	 = "";
    private String visatype 	 = "";
    private String visadate1	 = "";
    private String ckscheckout	 = "";
    private String specialcode	 = "";
    private String bplace_en	 = "";
    private String smsphone 	 = "";
    private String issuestate_app = "";
    private String docissdate_app = "";
    private String placeissue_app = "";
    private String last_name	  = "";
    private String first_name 	  = "";
    private String rptloc 	 	  = "";
    private String aldate 	 	  = "";
    
    
    public String getGroups()
    {
        return groups;
    }
    public void setGroups(String groups)
    {
        this.groups = groups;
    }
    public String getSection()
    {
        return section;
    }
    public void setSection(String section)
    {
        this.section = section;
    }
    public String getBirth()
    {
        return birth;
    }
    public void setBirth(String birth)
    {
        this.birth = birth;
    }
    public String getBlood()
    {
        return blood;
    }
    public void setBlood(String blood)
    {
        this.blood = blood;
    }
    public String getBplace()
    {
        return bplace;
    }
    public void setBplace(String bplace)
    {
        this.bplace = bplace;
    }
    public String getBplace_en()
    {
        return bplace_en;
    }
    public void setBplace_en(String bplace_en)
    {
        this.bplace_en = bplace_en;
    }
    public String getCabin()
    {
        return cabin;
    }
    public void setCabin(String cabin)
    {
        this.cabin = cabin;
    }
    public String getChgdate()
    {
        return chgdate;
    }
    public void setChgdate(String chgdate)
    {
        this.chgdate = chgdate;
    }
    public String getChguser()
    {
        return chguser;
    }
    public void setChguser(String chguser)
    {
        this.chguser = chguser;
    }
    public String getCity()
    {
        return city;
    }
    public void setCity(String city)
    {
        this.city = city;
    }
    public String getCkscheckout()
    {
        return ckscheckout;
    }
    public void setCkscheckout(String ckscheckout)
    {
        this.ckscheckout = ckscheckout;
    }
    public String getCmcno()
    {
        return cmcno;
    }
    public void setCmcno(String cmcno)
    {
        this.cmcno = cmcno;
    }
    public String getCname()
    {
        return cname;
    }
    public void setCname(String cname)
    {
        this.cname = cname;
    }
    public String getComments()
    {
        return comments;
    }
    public void setComments(String comments)
    {
        this.comments = comments;
    }
    public String getDeptno()
    {
        return deptno;
    }
    public void setDeptno(String deptno)
    {
        this.deptno = deptno;
    }
    public String getDocissdate_app()
    {
        return docissdate_app;
    }
    public void setDocissdate_app(String docissdate_app)
    {
        this.docissdate_app = docissdate_app;
    }
    public String getEducate()
    {
        return educate;
    }
    public void setEducate(String educate)
    {
        this.educate = educate;
    }
    public String getEmailadrs()
    {
        return emailadrs;
    }
    public void setEmailadrs(String emailadrs)
    {
        this.emailadrs = emailadrs;
    }
    public String getEmgname()
    {
        return emgname;
    }
    public void setEmgname(String emgname)
    {
        this.emgname = emgname;
    }
    public String getEmgphone1()
    {
        return emgphone1;
    }
    public void setEmgphone1(String emgphone1)
    {
        this.emgphone1 = emgphone1;
    }
    public String getEmgphone2()
    {
        return emgphone2;
    }
    public void setEmgphone2(String emgphone2)
    {
        this.emgphone2 = emgphone2;
    }
    public String getEmpn()
    {
        return empn;
    }
    public void setEmpn(String empn)
    {
        this.empn = empn;
    }
    public String getEname()
    {
        return ename;
    }
    public void setEname(String ename)
    {
        this.ename = ename;
    }
    public String getFirst_name()
    {
        return first_name;
    }
    public void setFirst_name(String first_name)
    {
        this.first_name = first_name;
    }
    public String getGreen()
    {
        return green;
    }
    public void setGreen(String green)
    {
        this.green = green;
    }
    public String getHeight()
    {
        return height;
    }
    public void setHeight(String height)
    {
        this.height = height;
    }
    public String getHiredate()
    {
        return hiredate;
    }
    public void setHiredate(String hiredate)
    {
        this.hiredate = hiredate;
    }
    public String getHomeadrs()
    {
        return homeadrs;
    }
    public void setHomeadrs(String homeadrs)
    {
        this.homeadrs = homeadrs;
    }
    public String getId()
    {
        return id;
    }
    public void setId(String id)
    {
        this.id = id;
    }
    public String getIndate()
    {
        return indate;
    }
    public void setIndate(String indate)
    {
        this.indate = indate;
    }
    public String getIssuestate_app()
    {
        return issuestate_app;
    }
    public void setIssuestate_app(String issuestate_app)
    {
        this.issuestate_app = issuestate_app;
    }
    public String getJobno()
    {
        return jobno;
    }
    public void setJobno(String jobno)
    {
        this.jobno = jobno;
    }
    public String getLast_name()
    {
        return last_name;
    }
    public void setLast_name(String last_name)
    {
        this.last_name = last_name;
    }
    public String getMailadrs()
    {
        return mailadrs;
    }
    public void setMailadrs(String mailadrs)
    {
        this.mailadrs = mailadrs;
    }
    public String getNation()
    {
        return nation;
    }
    public void setNation(String nation)
    {
        this.nation = nation;
    }
    public String getNewdate()
    {
        return newdate;
    }
    public void setNewdate(String newdate)
    {
        this.newdate = newdate;
    }
    public String getNewdept()
    {
        return newdept;
    }
    public void setNewdept(String newdept)
    {
        this.newdept = newdept;
    }
    public String getNewuser()
    {
        return newuser;
    }
    public void setNewuser(String newuser)
    {
        this.newuser = newuser;
    }
    public String getPassdate()
    {
        return passdate;
    }
    public void setPassdate(String passdate)
    {
        this.passdate = passdate;
    }
    public String getPassport()
    {
        return passport;
    }
    public void setPassport(String passport)
    {
        this.passport = passport;
    }
    public String getPhone1()
    {
        return phone1;
    }
    public void setPhone1(String phone1)
    {
        this.phone1 = phone1;
    }
    public String getPhone2()
    {
        return phone2;
    }
    public void setPhone2(String phone2)
    {
        this.phone2 = phone2;
    }
    public String getPhone3()
    {
        return phone3;
    }
    public void setPhone3(String phone3)
    {
        this.phone3 = phone3;
    }
    public String getPhone4()
    {
        return phone4;
    }
    public void setPhone4(String phone4)
    {
        this.phone4 = phone4;
    }
    public String getPhone5()
    {
        return phone5;
    }
    public void setPhone5(String phone5)
    {
        this.phone5 = phone5;
    }
    public String getPlaceissue_app()
    {
        return placeissue_app;
    }
    public void setPlaceissue_app(String placeissue_app)
    {
        this.placeissue_app = placeissue_app;
    }
    public String getProvince()
    {
        return province;
    }
    public void setProvince(String province)
    {
        this.province = province;
    }
    public String getResignno()
    {
        return resignno;
    }
    public void setResignno(String resignno)
    {
        this.resignno = resignno;
    }
    public String getRptloc()
    {
        return rptloc;
    }
    public void setRptloc(String rptloc)
    {
        this.rptloc = rptloc;
    }
    public String getRsndate()
    {
        return rsndate;
    }
    public void setRsndate(String rsndate)
    {
        this.rsndate = rsndate;
    }
    
    public String getSern()
    {
        return sern;
    }
    public void setSern(String sern)
    {
        this.sern = sern;
    }
    public String getSex()
    {
        return sex;
    }
    public void setSex(String sex)
    {
        this.sex = sex;
    }
    public String getSmsphone()
    {
        return smsphone;
    }
    public void setSmsphone(String smsphone)
    {
        this.smsphone = smsphone;
    }
    public String getSpecialcode()
    {
        return specialcode;
    }
    public void setSpecialcode(String specialcode)
    {
        this.specialcode = specialcode;
    }
    public String getSsdate()
    {
        return ssdate;
    }
    public void setSsdate(String ssdate)
    {
        this.ssdate = ssdate;
    }
    public String getStation()
    {
        return station;
    }
    public void setStation(String station)
    {
        this.station = station;
    }
    public String getStatus()
    {
        return status;
    }
    public void setStatus(String status)
    {
        this.status = status;
    }
    public String getVisadate()
    {
        return visadate;
    }
    public void setVisadate(String visadate)
    {
        this.visadate = visadate;
    }
    public String getVisadate1()
    {
        return visadate1;
    }
    public void setVisadate1(String visadate1)
    {
        this.visadate1 = visadate1;
    }
    public String getVisatype()
    {
        return visatype;
    }
    public void setVisatype(String visatype)
    {
        this.visatype = visatype;
    }
    public String getWorktype()
    {
        return worktype;
    }
    public void setWorktype(String worktype)
    {
        this.worktype = worktype;
    }
    public String getAldate()
    {
        return aldate;
    }
    public void setAldate(String aldate)
    {
        this.aldate = aldate;
    }
}
