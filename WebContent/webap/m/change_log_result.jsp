...
<%@page import="swap3ac.CrewSkjObj2"%>
<%@page import="swap3ac.*"%>
...
if(lObj == null ) {
	...
	if("A".equals(type)){
		...
		swap3ac.ShowSwapForm sformTPE = null;
		//swap3ackhh.ShowSwapForm sformKHH = null;
		swap3ac.SwapFormObj obj = null;
		//swap3ackhh.SwapFormObj obj2 = null;
		ArrayList aSwapSkjAL = null;
		ArrayList rSwapSkjAL = null;
		DecimalFormat df = new DecimalFormat("0000");
		int aALtimes = 0;
		int rALtimes = 0;
		if("TPE".equals(uObj.getBase()) && null != formno && !"".equals(formno)){
			sformTPE = new swap3ac.ShowSwapForm(formno);
			obj= sformTPE.getSwapFormObj();
		...
		/*obj = new SwapRdObj();
		obj.setFormnoAL((String)request.getParameter("formno"));
		obj.setFormtypeAL((String)request.getParameter("FormType"));
		obj.setaEmpnoAL((String)request.getParameter("aEmpno"));
		obj.setrEmpnoAL((String)request.getParameter("rEmpno"));
		obj.setaCnameAL((String)request.getParameter("aName"));
		obj.setrCnameAL((String)request.getParameter("rName"));
		obj.setCommentsAL((String)request.getParameter("comm"));
		obj.setCheckDateAL((String)request.getParameter("chkDate"));
		obj.setChgAllAL((String)request.getParameter("chgAll"));
		obj.setNewDateAL((String)request.getParameter("newDate"));*/
	
	/*else if("B".equals(type)){
		obj2 = new RealSwapRdObj(); 
		obj2.setAEmpno((String)request.getParameter("bAEmpno"));
		obj2.setREmpno((String)request.getParameter("bREmpno"));
		obj2.setAComm((String)request.getParameter("aComm"));
		obj2.setRComm((String)request.getParameter("rComm"));
		obj2.setACount((String)request.getParameter("aCount"));
		obj2.setRCount((String)request.getParameter("rCount"));
		obj2.setYear((String)request.getParameter("year"));
		obj2.setMonth((String)request.getParameter("month"));
		obj2.setChgDate((String)request.getParameter("chgDate"));
		obj2.setRCount((String)request.getParameter("chgUser"));
    }*/
%>
<!DOCTYPE html>
<html>
			...
			<ul class="result_content">
				<li>
					...
					%>
						...
						<br>
						<p style="font-size: 20px; font-weight: bold;"><%=skjObj.getDutycode() %><%if("77W".equals(skjObj.getActp())){out.print("&nbsp;"+skjObj.getActp());}%></p>
						<br>
...
<%
			...
			response.sendRedirect("khh/change_log_result.jsp?type=A&formno="+formno);			
		}
		else{
