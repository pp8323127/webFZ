...
<center>
  <p><font face="Comic Sans MS" color="#333333">Page Detail</font></p>
...
<%
   ...
   myResultSet = stmt.executeQuery("select * from DFTPAGE where pageid = '"+pageid+"' and groupcd = '"+groupcd+"'");
   if (myResultSet != null)
   {
        while (myResultSet.next())
   	{
   	        String auth = myResultSet.getString("auth");
	        String comments = myResultSet.getString("comments");
%>
  <form method="post" action="updpage.jsp">
    <table border="1">
      <tr> 
      ...
      <tr> 
        ...
        <td> 
          <input name="auth" type="text" width="10" value="<%= auth %>">
        </td>
      </tr>
      <tr> 
        <td><b><font face="Arial, Helvetica, sans-serif" size="2">Comments</font></b></td>
        <td> 
          <input name="comments" type="text" width="50" value="<%= comments %>">
        </td>
      </tr>
      <tr> 
        <td colspan="2"> 
	  ...
	  <tr> 
        <td colspan="2"> 
          <center>
            <font face="Arial, Helvetica, sans-serif" size="2"> <a href="delpage.jsp?pageid=<%=pageid%>&groupcd=<%=groupcd%>">Delete 
            Record </a> </font> 
          </center>
        </td>
      </tr> 
