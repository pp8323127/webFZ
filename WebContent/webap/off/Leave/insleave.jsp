...
//LeaveProgress lp = new LeaveProgress(userid, offsdate,offsdate,offtype,fltno,userid);
LeaveProgress lp = new LeaveProgress(userid, offsdate, offedate, offtype, occurdate, relation, fltno, userid);
...
<%
			...
			{
%>
			location.replace("../AL/viewoffsheet.jsp?offyear=<%=offsdate.substring(0,4)%>&offtype=a");
<%
			}
			else
			{
%>	
			location.replace("viewleavesheet.jsp?offyear=<%=offsdate.substring(0,4)%>&offtype=<%=vieqofftype%>");
<%
			}	
%>
		</SCRIPT>
...
<%
	...
	else
	{
		...
		try
		{
			...
			fwlog.write("Userid : "+userid+" Runtime: "+time1+" Empno : "+userid+" Offdate : "+offsdate+"~"+offedate+" Fltno : "+fltno+" Offtype : "+offtype+"  Error Msg : "+str2+"  \r\n");
			fwlog.write("*************************************** \r\n");	
		}
		catch (Exception e)
		{
...
else
{
	...
	try
	{
		java.util.Date runDate1 = Calendar.getInstance().getTime();
		String time1 = new SimpleDateFormat("yyyy/MM/dd EEE HH:mm:ss a").format(runDate1);
		fwlog.write("Userid : "+userid+" Runtime: "+time1+" Empno : "+userid+" Offdate : "+offsdate+"~"+offedate+" Fltno : "+fltno+" Offtype : "+offtype+"  Error Msg : "+str+"  \r\n");
		fwlog.write("*************************************** \r\n");	
	}
	catch (Exception e)
	{
