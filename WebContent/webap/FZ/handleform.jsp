...
sqlSB.append("select to_char(f.newdate,'yyyy/mm/dd hh24:mi') newD,f.*, 'Y' acount, '0' acomm, 'Y' rcount, '0' rcomm, 'A' formtype from fztform f where ");
...
sqlSB.append("select to_char(f.newdate,'yyyy/mm/dd hh24:mi') newD, f.*, 'B' formtype from fztbform f where ");
...
pstmt = conn.prepareStatement("select * from fztcomm ORDER BY Decode(citem,'Agree',' ',citem)");
...
while(rs.next())
{
	...
	swap3ac.SwapFormObj obj = new swap3ac.SwapFormObj();
...
<form name="form1" method="post" action="upd_handleform.jsp" ONSUBMIT="return checkForm()">
  ...
  換班申請單處理
    ...
    <%
	    ...
	    {
			swap3ac.SwapFormObj obj = (swap3ac.SwapFormObj)al.get(i);				
%>
  ...
  Reject</label>
      ...
      <td class="left" ><input name="addcomm" type="text" size="30" maxlength="30" style="margin-right:0.5em; ">
	  <select name="comm">
...
function showForm(formNo,formtype)
{
	...
	if(formtype=="A")
	{
		document.form2.action="swap3/showForm.jsp";
	}
	else
	{
		document.form2.action="credit/swaptpe/showBForm.jsp";
	}
