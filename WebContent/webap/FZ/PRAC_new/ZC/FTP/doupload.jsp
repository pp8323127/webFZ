...
//			example.setDirectory("/EGTEST/PR/");
			...
			rs = fp.updFile(fdate, fltno, dpt+arv, zcEmpno, newFileName, FileDescription);
...
%>
	...
	alert("檔案上傳成功!!");
	window.opener.location.href=self.location ="../zcedFltIrr.jsp?idx=<%=idx%>";
	self.close();
...
%>
	<p class='errStyle1'><%=msg%></p>
	 ...
	 <input type="button" name="Submit" value="重新上傳" onClick="self.location='uploadfile.jsp?fdate=<%=fdate%>&fltno=<%=fltno%>&dpt=<%=dpt%>&arv=<%=arv%>&zcEmpno=<%=zcEmpno%>&acno=<%=acno%>';">&nbsp;&nbsp;
