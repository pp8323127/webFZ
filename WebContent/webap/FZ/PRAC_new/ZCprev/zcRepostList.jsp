...
<%@ page import="java.sql.*,ci.db.*,java.util.ArrayList"%>
<%!
...
function viewRpt(fdate,fltno,dpt,arv,stdDt)
{
...
<form name="form1" method="post"><!--  action="/webfz/FZAC/PRAC/preCrewList.jsp">-->
	...
	<input type="hidden" name="sect" id="sect" value="">
	<input type="hidden" name="dpt" id="dpt" value="">
	<input type="hidden" name="arv" id="arv" value="">
	<input type="hidden" name="stdDt" id="stdDt" value="">
...
<caption class="center r"><%=year+month%>任務列表</caption>
  ...
  <tr  class="<%=bgColor%>">
    ...
    <td ><a href="javascript:viewRpt('<%=obj.getFdate()%>','<%=obj.getFltno()%>','<%=obj.getDpt()%>','<%=obj.getArv()%>','<%=obj.getStdDt()%>');"><img src="../../images/blue_view.gif" alt="Print Report" width="16" height="16"  border="0" title="Print Report"></a> </td>
    <td >
	<a href="javascript:viewCrewList('<%=obj.getFdate().substring(0,4)%>','<%=obj.getFdate().substring(5,7)%>','<%=obj.getFdate().substring(8)%>','<%=obj.getFltno()%>','<%=obj.getDpt()+obj.getArv()%>','<%=obj.getStdDt()%>');"><img src="../../images/format-justify-fill.png" width="16" height="16" border="0" alt="Print Crew List" title="Print Crew List"></a>
	</td>
  </tr>
<%
	}	
