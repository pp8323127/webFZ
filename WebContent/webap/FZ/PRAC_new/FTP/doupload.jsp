...
%>
	...
	alert("檔案上傳成功!!");
	window.opener.location.href="../edFltIrr<%=src%>.jsp?isZ=<%=isZ%>&fdate=<%=fdate%>&fltno=<%=fltno%>&dpt=<%=dpt%>&arv=<%=arv%>&purserEmpno=<%=purserEmpno%>&acno=<%=acno%>&pur=<%=purserEmpno%>";
//window.opener.location.href="../FltIrrList.jsp?fyy=<%=fdate.substring(0,4)%>&fmm=<%=fdate.substring(5,7)%>&fdd=<%=fdate.substring(8)%>&fltno=<%=fltno%>&GdYear=<%=fz.pracP.GdYear.getGdYear(fdate)%>";
	self.close();
