...
{//僅有一班時，check是否為本班次座艙長
	...
	else
	{
		...
		response.sendRedirect("edFltIrr"+src+".jsp?isZ=Y&fdate="+fyy+"/"+fmm+"/"+fdd+"&fltno="+obj.getFltno()+"&dpt="+obj.getDpt()+"&arv="+obj.getArv()+"&GdYear="+GdYear+"&acno="+obj.getAcno()+"&pur="+obj.getPurEmpno());
	}
}
//Betty add 
else
...
%>
  <tr bgcolor="<%=bgColor%>">
    ...
    <td align="center" valign="middle" class="tablebody"><div align="center"><a href="edFltIrr<%=src%>.jsp?isZ=<%=isZ%>&fdate=<%=obj.getStdD()%>&fltno=<%=gf.getFltnoWithSuffix()%>&dpt=<%=obj.getDpt()%>&arv=<%=obj.getArv()%>&GdYear=<%=GdYear%>&acno=<%=obj.getAcno()%>&pur=<%=obj.getPurEmpno()%>" target="_self"><img src="../images/ed3.gif" width="22" height="22" border="0" alt="Edit Report"></a></div></td>
	</tr>
  <%
	}
