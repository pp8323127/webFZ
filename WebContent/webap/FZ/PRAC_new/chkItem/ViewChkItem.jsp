...
<fieldset style="width:70% " ><legend>
...
<fieldset style="width:80% " >
	<legend>
	<input type="radio" name="executeStatus" value="Y" onClick="document.getElementById('exY').className='display';document.getElementById('exN').className='hidden';document.getElementById('esYComm').focus();" <%if(rdObj.isExecuteCheck()){out.print("checked");}%> disabled>已執行</legend>
	<div id="exY" class="<%if(rdObj.isExecuteCheck()){out.print("show");}else{out.print("hidden");}%>">
	<li>
	 	 <input type="radio" name="ev" value="Y" onClick="document.getElementById('evN').className='hidden';" <%if(rdObj.isExecuteCheck()&& rdObj.isEvalStatus()){out.print("checked");}%> disabled>執行一切正常
	</li>
	<li>
	<input type="radio" name="ev" value="N" onClick="document.getElementById('evN').className='display';" <%if(rdObj.isExecuteCheck()&& !rdObj.isEvalStatus()){out.print("checked");}%> disabled>執行未達標準
	</li>
	<br><br>
	<%
	...
	<div class="<%if(rdObj.isExecuteCheck()&&  !rdObj.isEvalStatus()){out.print("show");}else{out.print("hidden");}%>" id="evN">
		...
		for(int i=0;i<obj.getCheckDetailAL().size();i++)
		{
			...
			{
				if(rdObj.getCheckDetailAL() == null)
				{//無細項，顯示空白無勾選的選項
				
	%>
		<tr class="center<%if(i%2==1){out.print(" bg6");}%>" >
			<td><input type="checkbox" name="evItm" value="<%=dObj.getItemSeqno()%>" onClick="showCoi('coi<%=i%>',this.checked)" disabled></td>
	...
	%>
		<tr class="center<%if(i%2==1){out.print(" bg6");}%>">
			<td><input type="checkbox" name="evItm" value="<%=dObj.getItemSeqno()%>" <%if(tempHascheckItem){out.print("checked");}%> onClick="showCoi('coi<%=i%>',this.checked)" disabled></td>
			<td><input type="checkbox" name="correctItm" id="coi<%=i%>" value="<%=dObj.getItemSeqno()%>"  <%if(!tempHascheckItem){out.print("disabled");} if(tempisCorrect){out.print(" checked");}%> disabled></td>
			<td class="left"><%=dObj.getDescription()%></td>
		</tr>
	<%				
	...
	%>
		...
		<div style="margin-left:2em;">
			Comments：<br>
		<textarea name="esYComm" cols="40" rows="5" id="esYComm" readonly><%if(rdObj.isExecuteCheck()&& rdObj.getComments()!= null){out.print(rdObj.getComments());}%></textarea><br>
		</div>
	</div>
</fieldset>	
<fieldset style="width:80%; ">
	<legend><input type="radio" name="executeStatus" value="N" onClick="document.getElementById('exN').className='display';document.getElementById('exY').className='hidden';document.getElementById('esNComm').focus();" <%if(!rdObj.isExecuteCheck()){out.print("checked");}%> disabled>未執行</legend>
...
<textarea name="esNComm" cols="40" rows="5" id="esNComm" readonly><%if(!rdObj.isExecuteCheck()&& rdObj.getComments()!= null){out.print(rdObj.getComments());}%></textarea><br>
  </div>	
</fieldset>	
<br>
  </fieldset>
