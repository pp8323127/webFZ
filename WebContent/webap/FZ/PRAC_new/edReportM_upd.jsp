...
//out.println(shst1+"<br>"+shst2+"<br>"+shet1+"<br>"+shet2+"<br>"+shift+"<br>"+shreason);
//for(int i =0 ;i < sh_crew.length ;i++){
//	out.println(sh_crew[i]);
//}
//out.print(dpt+arv);
...
insertColumnValue = CfltCol.getColValue(empno,sern,crew,score,sh_crew);//新增輪班
...
updValue = CfltCol.getUPDValue(empno,sern,crew,score,duty,sh_crew);
...
+"SH_REMARK, SHIFT,SH_CM,MP_EMPN,SH_MP"
...
","+ insDutyValue 
+"'"+shreason+ "','" + shift + "','"+sh_cm+"','"+mp_empn+"','"+sh_mp+"'";
	...
	<noscript>
      ...
      <tr>
        <td colspan="3" valign="left">
          ...
          <span class="txtblue">Cabin's Report&nbsp; &nbsp;</span><span class="red12"><strong> Give In-Flight Service Grade  to each crew</strong></span></div></td>
      </tr>
      <tr>
...
<legend class="txttitletop">Crew Shift Records</legend>
	...
	<tr>  
	  ...
	  <br>
	   ...
	   <br>
	   <%if(null!=mp_empn && !"".equals(mp_empn)){ %>
	   <span class="txtblue">MP輪休梯次：<%=sh_mp%></span>
	   <%} %>
	  </td>	 
	</tr>	
	</table>
...
%>
      <tr bgcolor="<%=bcolor%>">
        <td height="27" class="tablebody"><input type="text" style="background-color:<%=bcolor%> ;border:0pt" readonly  size="12" value="<%=crew[i]%>" name="cname">
        </td>
        ...
        <td class="tablebody"><input type="text" style="background-color:<%=bcolor%> ;border:0pt;text-decoration:underline;cursor:hand" readonly  size="6" value="<%=sern[i]%>" name="sern" onClick="subwinXY('edGdType.jsp?sern=<%=sern[i]%>&cname=<%=URLEncoder.encode(crew[i])%>&fltno=<%=fltno%>&fdate=<%=fdate%>&s=<%=sect%>&g=<%=GdYear%>&empno=<%=empno[i]%>','edGdType','800','500')">
        </td>
...
%></td>
	  ...
	  if("ZC".equals(duty[i])){
	  /*
	  	linkstring = "edZC.jsp?sern="+sern[i]+"&fltno="+fltno+"&fdate="+fdate+"&s="+sect+"&empno="+empno[i];
		*/
	  %>
	  ...
	  <td class="tablebody"><input type="text" style="background-color:<%=bcolor%> ;border:0pt" readonly  size="3" value="<%=sh_crew[i]%>" name="sh_crew"></td>
...
&nbsp;&nbsp;&nbsp;<span class="txtblue">
		...
		<!--ZC-->
		  ...
		  <input type="hidden" name="dpt" name="dpt" value="<%=dpt%>">
