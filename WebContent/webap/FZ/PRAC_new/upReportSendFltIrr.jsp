...
{
	//get pax 人數
	pax_count = fd.getPaxCount(fdate, fltno, sect); 
	//get 彈派人數
	disp_count = fd.getFlexibleNum(fltno, tempfleet, sect, pax_count) ;
	//get ACM 人數
	acm_count = fd.getACMCount(fdate, fltno, sect) ;
...
String shift = chk.getChickShift(fdate, fltno, sect);
//******************************************************************
//若無輸入登機資料，不得送出報告!!
...
String goToPage= "flightcrew.jsp?fyy="+fdate.substring(0,4)+"&fmm="+fdate.substring(5,7)+"&fdd="+
			fdate.substring(8)+"&fltno="+fltno+"&acno=null";
%>
<script language="javascript" type="text/javascript">
...
String goToPage= "upReportSave.jsp?fdate="+fdate+"&purserEmpno="+purserEmpno+"&psrsern="+psrsern+"&fltno="+fltno+"&dpt="+sect.substring(0,3)+"&arv="+sect.substring(3)+"&acno=null&ispart2=N&chk=R";

%>
<script language="javascript" type="text/javascript">
alert("尚未填寫客艙經理自我督察報告,不得送出!!\n");
self.location = "<%=goToPage%>";
//window.history.back(-1);
</script>
<%
...
%>
<script language="javascript" type="text/javascript">
alert("DFA人數不正確,請確認,或於<Flt Irregularity>中註明原因!!\n");
self.location = "<%=goToPage%>";
</script>
<%
//SR 3086 
}else if(shift.equals("Y")){
String goToPage= "edReportModify.jsp?fdate="+fdate+"&fltno="+fltno+"&dpt="+sect.substring(0,3)+"&arv="+sect.substring(3,6);
//out.println(goToPage);
out.println(chk.getMsg());
%>

<script language="javascript" type="text/javascript">
var msg = self.document.valueOf(<%=chk.getMsg()%>);
alert(msg);
self.location = "<%=goToPage%>";
//window.history.back(-1);
</script>
...
if(rowCount > 0)
{
	...
	stmt.executeUpdate(sql);//將upd設為N 己送出
...
*/	
	...
	if(fltd == null)
	{
		sql = "insert into egtcrpt(fltd,fltno,sect,empno,chguser,chgdate,flag,caseclose,newdate,trip_num) values(to_date('"+ fdate +"','yyyy/mm/dd'),'"+fltno+"','"+sect+"','"+sGetUsr+"','"+sGetUsr+"',sysdate,'Y',null,sysdate,"+trip_num+")";
	}
	else
	{
		if(newdate == null)
...
{	//無報告紀錄，到flightcrew，傳遞參數：fyy,fnn,fdd,fltno,GdYear,acno
	//11~12月的航班，考績年度為下一個年度
   if("11".equals(fdate.substring(5,7)) || "12".equals(fdate.substring(5,7)))
	{	
	...
	goPage = "flightcrew.jsp?fyy="+fdate.substring(0,4)+"&fmm="+fdate.substring(5,7)+"&fdd="+
			fdate.substring(8)+"&fltno="+fltno+"&GdYear="+GdYear+"&acno=null"+"&sect="+sect;
...
<body>
<input type="hidden" id=msg name = msg value=<%=chk.getMsg()%>/>
<%
if(t)
{	  response.sendRedirect("../showmessage.jsp?messagestring="+URLEncoder.encode("系統忙碌中，請稍後再試"));	
}
