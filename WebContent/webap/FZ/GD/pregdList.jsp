...
<%
			...
			{	
%>
		  <a href="gdList.jsp?sel_year=<%=yyyy%>&sel_mon=<%=mm%>&sel_dd=<%=dd%>&fltno=<%=fltno%>&dpt=<%=dpt%>&ftime=<%=obj.getFdate().substring(11)%>" target="_blank"><span  class="txtxred"><%=obj.getFdate().substring(11)%></span>
		  </a>
<%
			}
			else
			{			  
%>
		  <a href="gdList_J.jsp?sel_year=<%=yyyy%>&sel_mon=<%=mm%>&sel_dd=<%=dd%>&fltno=<%=fltno%>&dpt=<%=dpt%>&ftime=<%=obj.getFdate().substring(11)%>" target="_blank"><span  class="txtxred"><%=obj.getFdate().substring(11)%></span>
		  </a>
<%
			}
%>
