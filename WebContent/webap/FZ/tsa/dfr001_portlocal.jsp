...
String mysql = "select to_char(b.da13_stdl,'mm') mm, c.empno empno, a.c_name name, a.ac_type fleet, a.job_type occu, " +
...
"and to_char((b.da13_stdl),'yyyy') = '"+year+"' " +  whereString +
"and substr(c.empno,1,1) <> '8' " +
"and ((to_date(f.year||f.mon||f.dd||f.blkout,'yyyymmddHH24MI') - b.da13_etdu) * 24 >= -2 " +  
"and (to_date(f.year||f.mon||f.dd||f.blkout,'yyyymmddHH24MI') - b.da13_etdu) * 24 <= 2) " +  
"group by a.ac_type, a.job_type, c.empno, to_char(b.da13_stdl,'mm'), a.c_name";
//out.println(mysql);
//DataSource
...
%>
<div class="txtblue"><font face="Comic Sans MS" size="3"><%= year %> Flight Crew BLK Time Report(Log - Port Local Time / 員額表)</font></div>
<br>
...
//cs66 2006/2/3改，原   String fleet = myResultSet.getString("fleet");
...
%>
    ...
    <% 
		...
		oldempno = empno;
		//cs66 2006/2/3改，原 oldfleet = fleet;
		oldfleet = fleet2;
		oldoccu = occu;
		oldname = name;
		timesum = 0;
