...
Hashtable htInrosdt     = new Hashtable();
...
if (rank.equals("RP+FO")) rank = "RP', 'FO";
...
if ("PC".equals(checkType)) 
	 ...
	 trnCdCond = " AND SUBSTR(rostrg.TRG_CD,1,1)='0' AND " +
...
if (tableSource.equals("DZ_DF")) {     
    ...
    try{
		...
		// set rs scrollable for using rs.last...etc	
        ...
        ArrEmpno  = new ArrayList(); ArrItemname = new ArrayList(); ArrCname   = new ArrayList();   
		ArrActype = new ArrayList(); ArrJobtype  = new ArrayList(); ArrEname   = new ArrayList();   
		ArrActp   = new ArrayList(); ArrCdate    = new ArrayList(); ArrCkadjdt = new ArrayList();			
        //out.print(sql+"<br>");

        myResultSet.last();
        if (myResultSet.getRow() > 0) rsNotNull = true; else rsNotNull = false;
	    ...
	    //if( myResultSet != null ){ 		
          ...
          out.println("SQL Exception Error : <BR>"+ sql+ "<BR>" +e.toString());
    }catch (Exception e){
          out.print("Exception Error(1) :  <BR>" + sql+ "<BR>ArrEmpno.size()=" +ArrEmpno.size() +"<BR>"+ e.toString());
 	}finally{
...
/*********************************/
    ...
    }catch (Exception e){    out.println("Exception Error(2) :  <BR>" + MSSQL_sql + "\r\n" + e.toString());
    }finally{
	...
	try {	
		...
		// set rs scrollable for using rs.last...etc
	    stmt_2 = conn_2.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);	 
		...
		rs_2.last();
        if (rs_2.getRow() > 0) rsNotNull = true; else rsNotNull = false;
        rs_2.beforeFirst();
		...
		//if(rs_2 != null){
	       while (rs_2.next()){ 
			   ...
			   empnoList.append("'"+rs_2.getString("empno")+"',");
...
try {	
	 ...
	 // set rs scrollable for using rs.last...etc
	 stmt_ratexp = conn_ratexp.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);	 
     ...
     rs_ratexp.last();
     if (rs_ratexp.getRow() > 0) rsNotNull = true; else rsNotNull = false;	
     rs_ratexp.beforeFirst();
...
}catch (SQLException e) { out.println("SQLException..ratexp: <BR>" +sql_ratexp +"<BR>"+ e.toString());
...
try {	
	 ...
	 sql_4 = "select r.staff_num, TO_CHAR(r.str_dt,'yyyy-mm-dd') str_dt " +
			 ...
			 "WHERE r.series_num = rostrg.SERIES_NUM " +
			   ...
			   "and r.staff_num in ("+empnoList.toString()+")  " +
			   trnCdCond +
	 ...
	 if(rs_4 != null){
		while (rs_4.next()){ 
		      htInrosdt.put(rs_4.getString("staff_num"), rs_4.getString("str_dt"));
		}//while
	 }//if
...
if (ArrEmpno.size() == 0){
     ...
     <% 
	  ...
	  for  (i = 0; i < ArrEmpno.size(); i++){ 
		 ...
		 <% inrosdt = (String)htInrosdt.get(currEmpno); %>
         <td class="FontSizeEngB"><%=inrosdt%></td>
