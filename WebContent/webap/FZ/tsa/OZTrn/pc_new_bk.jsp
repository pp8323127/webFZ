...
String syyyymm = request.getParameter("sel_year1") + request.getParameter("sel_mon1");
...
//out.print("~~~"+syyyymm+"~~~"+eyyyymm+"~~~");
boolean status=  false;
String  errMsg = "";
RetrieveCheckPCData2 rd = new RetrieveCheckPCData2(fleet,rank,syyyymm,eyyyymm);
...
<table width="90%"  border="0" cellpadding="1" cellspacing="1" align="center">
  ...
  <tr><td><span class="fonte_dblue2">Fleet:<span class="txtred"><strong><%=fleet%></strong></span> Rank:<span class="txtred"><strong><%=rank%></strong></span> </span>
...
<table width="90%"  border="1" cellpadding="1" cellspacing="1" bordercolor="#CCCCCC" align="center">
  <tr class="tablehead4">
    ...
    <td><div align="center">CheckType</div></td>
  </tr>
  <%
  int rowCount = 1;
...
%>
  <tr bgcolor="<%=bgColor%>">
    <td><div align="right">
        <input type="checkbox" value="<%=i%>" name="sel" onClick="calc()"><%=rowCount%></div>
    </td>
	...
	try {	
		...
		sql = "select ckadjdt from (select to_char(ckadjdt,'yyyy-mm-dd') ckadjdt from dzdb.dztckaj where empno='"+ck.getEmpno()+"' order by newdate desc) where rownum=1 ";
