...
%>
  <tr bgcolor="<%=bgColor%>">
	...
	<td><div align="center"><%=ck.getChktype()%> 
	<%
	dualRating = "N";
	try {	
		...
		sql_2 = "select count(*) cnt from fztckpl where empno='"+ck.getEmpno()+"' ";
		rs_2 = stmt_2.executeQuery(sql_2); 		
		if(rs_2 != null){
	   		while (rs_2.next()){ dualRating = rs_2.getString("cnt");  }//while
		}//if	
			
	}catch (SQLException e) { out.println("SQLException..03: " + e.toString());
	}catch (Exception e)    { out.println("Exception..03: ");
	}finally { if ( conn_2 != null ) try {	conn_2.close();  }catch (SQLException e) {}
	}//try
	
	//Display SIM check type if dual rating
	if (dualRating.equals("2")) { 
	   %>(<%=ck.getActp()%>)<%
	}//if 
	%>
	</div></td>
	...
	if (dualRating.equals("2")) { 
	    if (ck.getActp().equals("333")) %> (343) <%
		else if (ck.getActp().equals("343"))%> (333) <%
		else %>&nbsp;<%
	}//if 
	%>
