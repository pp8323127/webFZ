...
public class CrewListOfDailyCheckObj {
	...
	private String lastFlt = "";//CS80 2013-6 SR3081
	...
	public String getLastFlt() { //CS80 2013-6 SR3081
		return lastFlt;
	}
	public void setLastFlt(String lastFlt) { //CS80 2013-6 SR3081
		this.lastFlt = lastFlt;
	}

	public String getAct_str_dt_tm_gmt() { //CS80 2013-6 SR3081
		return act_str_dt_tm_gmt;
	}
	public void setAct_str_dt_tm_gmt(String act_str_dt_tm_gmt) { //CS80 2013-6 SR3081
...
sb.append("NVL(to_char(v.expiry_dts, 'yyyy/mm/dd'),'NULL') chn_visa_exp_dt, "); //CS40 2010-12-29 SR0416
sb.append("to_char(dps.act_str_dt_tm_gmt,'yyyymmddhh24mi') actStrDt ");//CS80 2013-6 SR3081
...
sb.append("crew_qualifications_v v "); //CS40 2010-12-29 SR0416
...
sb.append("and lpad(dp.flt_num(+),5,'0') =lpad(?,5,'0')  "); //CS40 2010-12-23 five digit flt_num(0511Z)
sb.append("and v.qual_cd(+) = 'CHN' "); //CS40 2010-12-29 SR0416
...
while (rs.next()) {
	...
	obj.setAct_str_dt_tm_gmt(rs.getString("actStrDt"));//CS80 2013-6 SR3081
	al.add(obj);
...
//取得 Credit Hour
	for (int i = 0; i < al.size(); i++) {
		...
		rs = pstmt.executeQuery();
		if (rs.next()) {
			obj.setCr(ci.tool.TimeUtil.minToHHMM(rs.getString("totalcr")));
		}
		rs.close();
		pstmt.close();
		
		//out.println(obj.getAct_str_dt_tm_gmt());
//get Last flt
		...
		pstmt.setString(3,  obj.getTripNo());
		rs =pstmt.executeQuery();
		if(rs.next()){
			obj.setLastFlt(rs.getString("lastfly"));
		} 
		rs.close();
		pstmt.close();
//
...
<html>
<head>
<meta http-equiv="refresh" content="30;url=showDailyC.jsp;text/html; charset=big5">
<title>Crew</title>
...
<table width="90%"  border="1" align="center" cellpadding="0" cellspacing="0" class="tablebody" style="empty-cells:show;border-collapse:collapse;border-color:#666666;margin-top:2em; ">
 ...
 <%
  ...
  %>
    <caption class="txtxred" align="center"><font size="+1">
	*RptLoc 為‘TAO’表示桃園報到組員<BR>
	*CHN Visa背景為紅色組員代表台包簽證已過期者<BR>
    *姓名背景為紅色組員代表AirCrews Check In系統未報到者
  </font></caption>  
  <%
  }
...
<%
  ...
  <tr  class="txtblue"  style='background-color:<%=bg%>' >
    ...
    <td><%
	//2013-1-3 CS40
	if ("PR".equals(obj.getRank()) ) out.print("CM"); // PR ==> CM
	else if ("FC".equals(obj.getRank()) ) out.print("PR"); // FC ==> PR(PU)
	else out.print(obj.getRank());	
	%></td>
...
<div align="center">
  ...
  <strong><font color="#FF0000" size="+2">檢視完畢後, 請記得按Back鍵返回</font></strong> =&gt; 
