...
try{
	...
	"and r.staff_num='"+sGetUsr+"' and d.duty_cd='FLY' and r.delete_ind='N' and d.act_port_a='TPE' " +
...
for(int i=0; i < str_dt_tpe_c.size(); i++){
	...
	sql = "select nvl(crewchk,'N') crc_ck from fztckin where fltd=to_date('"+fltdate.get(i)+
...
%>
	<tr class="tablebody">
		...
		//if(("Y".equals(no_show) && "N".equals(crc_ck)) || "634990".equals(sGetUsr)){
			if(f != null){
				%>
				<td><div align="center"><a href="check_menu.jsp?fltdate=<%=fltdate.get(i)%>&fltno=<%=fltno.get(i)%>&sector=<%=sect.get(i)%>&fleet=<%=fleet.get(i)%>&rank=<%=rank.get(i)%>"><img src="../images/ed1.gif" alt="Check In" width="22" height="22" border="0"></a></div></td>
  				<%
			}
			else{
				%>
