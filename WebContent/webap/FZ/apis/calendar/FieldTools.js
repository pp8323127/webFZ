...
function Calendar(sPath,t,x,y) {
	//var sPath = "calendar.htm";
	//var s = eval(sPath);
	strFeatures = "dialogWidth=206px;dialogHeight=210px;center=yes;help=no;status=no;dialogLeft="+x+";dialogTop="+y;
	st = t.value;
...
function formatDate(sDate) {
	...
	if (iMon.toString().length<2){ 
		iMon="0" + iMon.toString()
	}
	if (iDay.toString().length<2){ 
		iDay="0" + iDay.toString()
	}
	sScrap = iYea + "/" + iMon  + "/" + iDay ;
	//sScrap = iYea.toString() + iMon.toString()  + iDay.toString() ;
	//sScrap = iMon +'/'+ iDay.toString()+'/'+ iYea.toString() ;
	return sScrap;
}
