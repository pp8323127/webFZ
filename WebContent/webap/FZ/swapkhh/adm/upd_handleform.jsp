...
try 
{
	...
	for(int i=0;i<formno.length;i++)
	{
		...
		{
			sql = "update fztformf set ed_check='"+cf[i]+"', comments='"+addcomm[i]+" "+comm[i]+"', checkuser='"+userid+"', checkdate=sysdate where formno='"+formno[i]+"'";
		}
		else
		{
			sql = "update fztbformf set ed_check='"+cf[i]+"', comments='"+addcomm[i]+" "+comm[i]+"', checkuser='"+userid+"', checkdate=sysdate where formno='"+formno[i]+"'";
		}
		//out.println(sql+"<br>");
...
//寄送 email
        ...
        MimeMessage msg = new MimeMessage(mailSession);
		 ...
		 for (int i = 0; i < formno.length; i++) 
		 {
			...
			InternetAddress sender = new InternetAddress("khhefcibox@email.china-airlines.com");
			sender.setPersonal("KHHEF", "Big5");
