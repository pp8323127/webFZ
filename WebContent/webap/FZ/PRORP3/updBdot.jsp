...
//登機準時資訊
	borot = new fz.pr.orp3.BordingOnTime(fdate,fltno,sect,purserEmpno);
	try {
		borot.SelectData();
...
if ( !borot.isHasFlightInfo()){	//insert 

	if("Y".equals(bdot)){ 
	
		sql = "insert into egtcflt (fltd,fltno,sect,acno,psrempn,psrsern,psrname,pgroups,chgdate,bdot,bdtime,bdreason) "
			+"values(to_date('"+fdate+" 0000','yyyy/mm/dd hh24mi'),?,?,?,?,?,?,?,sysdate,?,to_date('"
			+bdtime+"','yyyymmdd hh24mi'),null)";
...
//	  out.print(e.toString());
	flag =-1;
	msg =e.toString();//"登機時間格式錯誤，請輸入HHMM<br>或備註字數超過75個中文字.";
	System.out.print(e.toString());
}catch (Exception e){
	flag =-1;
	msg = e.toString();
//	  out.print(e.toString());
	...
	String goPage = "edFltIrr.jsp?purserEmpno="+purserEmpno+"&fdate="+fdate+"&fltno="+fltno
			+"&dpt="+sect.substring(0,3)+"&arv="+sect.substring(3)+"&acno="+acno+"&GdYear="
