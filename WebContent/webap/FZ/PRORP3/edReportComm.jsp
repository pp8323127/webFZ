...
int nGdYear = 0;
try{
	GdYear = "2005";//request.getParameter("GdYear");
	//if(GdYear == null || "".equals(GdYear)) GdYear = "2005";
	//nGdYear = 1000+Integer.parseInt(GdYear);
//	nGdYear=1000+2005;
	nGdYear=2005;
}catch (Exception e)
{
...
for(int i=0; i<duty.length; i++){
	for(int j=0; j<duty.length; j++){
		...
		if(duty[i].toUpperCase().equals(duty[j].toUpperCase()) && i != j && !duty[i].equals("X")){
