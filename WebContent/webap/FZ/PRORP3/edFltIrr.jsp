...
<%@ page import="ci.db.*,fz.*,java.sql.*,java.net.URLEncoder,fz.pr.orp3.*,java.util.ArrayList" %>
<%
...
//登機準時資訊
fz.pr.orp3.BordingOnTime borot = new fz.pr.orp3.BordingOnTime(fdate,fltno,dpt+arv,purserEmpno);
try {
	borot.SelectData();
...
%>
		...
		document.getElementById("bdreason").value="<%=borot.getBdReason()%>";		
	showBt(2);
<%		}
	%>
...
%>
  ...
  <table width="78%"  border="0" align="center" cellpadding="2" cellspacing="0" class="fortable">
    ...
    <tr class="fortable">
      ...
      <td width="10%" class="fortable txtblue" align="left" ><a href="#" onClick="subwinXY('edFltIrr2.jsp?yearsern=<%=yearsern%>&purserEmpno=<%=purserEmpno%>&pur=<%=purserEmpno%>','','700','350')"><u><%=itemNoDsc%></u></a></td>
...
%>
<div align="center">
  <input name="upload" type="button" class="addButton" value="Upload(上傳檔案)" onClick="subwinXY('FTP/uploadfile.jsp?fdate=<%=fdate%>&fltno=<%=fltno%>&dpt=<%=dpt%>&arv=<%=arv%>&purserEmpno=<%=purserEmpno%>&acno=<%=acno%>','upload','700','350')">
</div>
<hr noshade>
<%
...
&nbsp;&nbsp;<br><span style="background-color:#3366CC;color:#FFFFFF;font-size:10pt;padding:2pt;font-family:Verdana ">
...
<tr>
  <td width="2%">&nbsp;</td>
  <td width="98%"><span class="txtblue">登機準時：
      <label>
...
<tr>
  <td colspan="2">
	...
	<input type="reset" value="Reset">
		...
		<input type="hidden" name="purserEmpno" value="<%=purserEmpno%>">	
		<input type="hidden" name="psrname" value="<%=ck.getName()%>">	
		<input type="hidden" name="psrsern" value="<%=ck.getSern()%>">
		<input type="hidden" name="pgroups" value="<%=ck.getGroup()%>">
		  <input type="hidden" name="GdYear" value="<%=GdYear%>">
...
<%
/*
out.println("<!--第一層選單的預設值 -->");
out.println("<option value=\"旅客\">旅客</option>");
out.println("<option value=\"硬體缺失\">硬體缺失</option>");
out.println("<option value=\"空服用品缺失\">空服用品缺失</option>");
out.println("<option value=\"餐點缺失\">餐點缺失</option>");
out.println("<option value=\"運務及訂位異常\">運務及訂位異常</option>");
out.println("<option value=\"客艙清潔\">客艙清潔</option>");
out.println("<option value=\"飛安事件及飛安查核\">飛安事件及飛安查核</option>");
out.println("<option value=\"其它及建議事項\">其它及建議事項</option>");
out.println("<option value=\"組員相關\">組員相關</option>");
out.println("<option value=\"班機延誤\">班機延誤</option>");
out.println("<option value=\"旅館相關事務\">旅館相關事務</option>");
*/
%>
		</select> 
...
<%
/*
out.println("<!--第二層選單的預設值 -->");
out.println("<option value=\"升等事宜\">升等事宜</option>");
out.println("<option value=\"會員資料更新\">會員資料更新</option>");
out.println("<option value=\"MVC\">MVC</option>");
out.println("<option value=\"旅客財物損失\">旅客財物損失</option>");
out.println("<option value=\"旅客酒醉\">旅客酒醉</option>");
out.println("<option value=\"遭行李打傷\">遭行李打傷</option>");
out.println("<option value=\"遭熱飲燙傷\">遭熱飲燙傷</option>");
out.println("<option value=\"遭service cart撞傷\">遭service cart撞傷</option>");
out.println("<option value=\"其他\">其他</option>");
out.println("<option value=\"旅客生病\">旅客生病</option>");
out.println("<option value=\"旅客違規吸煙\">旅客違規吸煙</option>");
out.println("<option value=\"違規使用電子產品(包括行動電話)\">違規使用電子產品(包括行動電話)</option>");
out.println("<option value=\"非理性行為\">非理性行為</option>");
out.println("<option value=\"性騷擾\">性騷擾</option>");
out.println("<option value=\"遭餐飲潑灑\">遭餐飲潑灑</option>");
*/
%>	        </select>
			<select  name="item3">
